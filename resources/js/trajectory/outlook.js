import { cashTimeline, incomeSeries, monthlyBalances, actualProjectedBalances, incomeRunwaySeries, previousYearSeries } from './chart-series.js';
...
export function outlookUI({getData,esc,money,precise,date,plot,chart,line,btn,dialog,evidence,forecastDetail}) {
  ...
  let horizon=365,view='bars',monthlyView='runway',incomeSource='last-year',previousMetric='spending';
  ...
  function html(scenarioId,seasonal) {
    ...
    const comparisonYear=Number(data.range.end.slice(0,4));
    const previous=previousYearSeries(data.history?.months||[],comparisonYear,previousMetric);
    const comparisonLabels={spending:'Spending',income:'Income',net:'Income after spending'};
    const comparisonDelta=previous.comparableCurrentTotal-previous.comparablePreviousTotal;
    return `<section class="tr-panel tr-outlook" aria-labelledby="tr-outlook-title">
      ...
      <div class="tr-outlook-tabs" role="tablist" aria-label="Outlook chart view">${[['lines','Trend lines'],['bars','Month by month'],['comparison','Actual vs. projected'],['previous','Previous year']].map(([key,label])=>`<button id="tr-outlook-${key}" role="tab" aria-selected="${view===key}" aria-controls="tr-outlook-charts" tabindex="${view===key?0:-1}" data-action="outlook-view" data-view="${key}">${label}</button>`).join('')}</div>
      <div id="tr-outlook-charts" class="tr-outlook-grid ${(view==='comparison'||view==='previous'||(view==='bars'&&monthlyView==='runway'))?'tr-outlook-comparison':''}" role="tabpanel" aria-labelledby="tr-outlook-${view}">
        <section class="tr-outlook-cash" aria-labelledby="tr-cash-title"><div class="tr-chart-heading"><div><h3 id="tr-cash-title">${view==='previous'?`${comparisonLabels[previousMetric]} by month`:view==='comparison'?'Actual vs. projected cash':view==='bars'?(monthlyView==='runway'?'Income runway':'Cash by month'):'Cash outlook'}</h3><p>${view==='previous'?`${comparisonYear} compared with ${comparisonYear-1}`:view==='comparison'?'Recorded balances and future month-end estimates':view==='bars'?(monthlyView==='runway'?'Expected income and cash outflows · next 12 months':'Projected closing balances'):'Balance after bills & spending'} · USD</p></div>${view==='previous'?`<div class="tr-segmented" role="group" aria-label="Previous year comparison metric">${[['spending','Spending'],['income','Income'],['net','Income after spending']].map(([key,label])=>`<button data-action="previous-metric" data-metric="${key}" aria-pressed="${previousMetric===key}">${label}</button>`).join('')}</div>`:view==='bars'&&monthlyView==='runway'?'':`<div class="tr-horizon" role="group" aria-label="Cash forecast horizon">${[[30,'30D'],[90,'90D'],[365,'1Y']].map(([n,label])=>`<button data-action="forecast-horizon" data-days="${n}" aria-pressed="${horizon===n}">${label}</button>`).join('')}</div>`}</div>
        ...
        ${plot('tr-forecast',view==='previous'?`${comparisonLabels[previousMetric]} by calendar month for ${comparisonYear} and ${comparisonYear-1} in USD`:view==='comparison'?'Actual and projected monthly cash balances in USD':view==='bars'?(monthlyView==='runway'?'Projected income, same-period-last-year income, and expected cash outflows in USD':'Projected closing cash balances by month in USD'):'Observed and projected cash balances in USD',view==='previous'?'outlook-previous':'outlook-cash')}
        <div class="tr-cash-result">${view==='previous'?`<div><span>${comparisonYear} · ${previous.comparableCount} completed matches</span><strong>${money(previous.comparableCurrentTotal)}</strong></div><div><span>${comparisonYear-1} · same ${previous.comparableCount} months</span><strong>${money(previous.comparablePreviousTotal)}</strong></div><div><span>Difference · matching months</span><strong class="${comparisonDelta>0&&previousMetric==='spending'?'tr-gap':''}">${money(comparisonDelta)}</strong></div>`:view==='bars'&&monthlyView==='runway'?`<div><span>Projected income · next 12 months</span><strong>${money(expectedIncome)}</strong></div><div><span>Expected cash outflows · next 12 months</span><strong>${money(expectedOutflows)}</strong></div><div><span>First income gap</span><strong>${firstIncomeGap?date(firstIncomeGap.date):'None projected'}</strong></div>`:`<div><span>Available in ${horizon===365?'12 months':horizon+' days'}</span><strong>${money(end?.available_cents)}</strong></div><div><span>First projected shortfall · full year</span><strong>${data.forecast.first_shortfall_on?date(data.forecast.first_shortfall_on):'None projected'}</strong></div>`}</div>
        <p class="tr-chart-caption">${view==='previous'?`Grouped bars compare each calendar month with the same month one year earlier. Missing months stay blank. The current month is incomplete, and imported coverage can differ by account. Transfers, card payments, loan proceeds, and asset sales are excluded. Select a month to review its supporting transactions.`:view==='bars'&&monthlyView==='runway'?`Projected income currently uses ${incomeSourceLabel}. Change its source to compare the other evidence. Outflows include scheduled bills, debt payments, and estimated cash spending; transfers, asset sales, loan draws, and goal reserves are kept separate. This comparison does not silently add a new income assumption to the cash-balance forecast.`:view==='comparison'?'Actual bars use the last recorded balance in each month, with its date available on selection. The current month compares cash observed so far with its projected closing balance—not two amounts to add together. Missing observations stay blank. These are today’s forward estimates, not archived forecasts.':`${view==='bars'?'Bars use the final projected day in each month; first and last months may be partial. ':''}${availableSame?`Cash and available cash are equal; shown as one ${view==='bars'?'series':'line'}.`:'Available cash subtracts money reserved for goals and medical bills.'}`} Select a ${view==='lines'?'point':'bar'} to see its details.</p>
        </section>
        ${p&&view!=='comparison'&&view!=='previous'&&!(view==='bars'&&monthlyView==='runway')?`<section class="tr-outlook-income" aria-labelledby="tr-income-title"><div class="tr-chart-heading"><div><h3 id="tr-income-title">Income vs. what you need</h3><p>Monthly rate · next 12 months · USD</p></div></div>
  ...
  function draw() {
    ...
    if(view==='previous') {
      const year=Number(data.range.end.slice(0,4)),comparison=previousYearSeries(data.history?.months||[],year,previousMetric);
      const labels=Array.from({length:12},(_,index)=>new Date(2000,index,1,12).toLocaleDateString('en-US',{month:'short'}));
      chart('tr-forecast','bar',labels,[
        bar(String(year),comparison.current,'#187a70'),bar(String(year-1),comparison.previous,'#9fb4c0'),
      ],i=>{
        const row=comparison.rows[i],label={spending:'spending',income:'income',net:'income after spending'}[previousMetric];
        dialog(`${labels[i]} · previous year comparison`,`<div class="tr-block"><div class="tr-row">${year} ${label}${row.current?.closed_month===false?' · month in progress':''}<strong>${row.current_cents===null?'No imported evidence':precise(row.current_cents)}</strong></div><div class="tr-row">${year-1} ${label}<strong>${row.previous_cents===null?'No imported evidence':precise(row.previous_cents)}</strong></div><div class="tr-row">Difference<strong>${row.current_cents===null||row.previous_cents===null?'Not comparable':precise(row.current_cents-row.previous_cents)}</strong></div><p class="tr-subtle">Amounts use reviewed classifications where available. Missing account coverage remains missing rather than becoming zero.</p><div class="tr-actions">${row.current?.ids?.length?btn(`View ${year} transactions`,'plan-evidence',`data-ids="${esc(JSON.stringify(row.current.ids))}"`):''}${row.previous?.ids?.length?btn(`View ${year-1} transactions`,'plan-evidence',`data-ids="${esc(JSON.stringify(row.previous.ids))}"`):''}</div></div>`,null);
      },{beginAtZero:previousMetric!=='net',showAllTicks:true});
      return;
    }
    if(view==='bars') {
  ...
  return {html,draw,setView(value){if(['lines','bars','comparison','previous'].includes(value))view=value;},setMonthlyView(value){if(['runway','cash'].includes(value))monthlyView=value;},setIncomeSource(value){if(['last-year','plan','recent'].includes(value))incomeSource=value;},setPreviousMetric(value){if(['spending','income','net'].includes(value))previousMetric=value;},setHorizon(value){if([30,90,365].includes(value))horizon=value;}};
}
