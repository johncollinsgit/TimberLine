...
function boot() {
  ...
  function render() {
    ...
    const views=scope==='both'?{overview:bothOverview,transactions}:{overview,accounts,history,budget,transactions,bills,medical,assets,business};
    const labels={overview:'Your trajectory',accounts:'Accounts',history:'Cash flow & history',budget:'Your budget',transactions:'Review transactions',bills:'Bills & goals',medical:'Medical sharing',assets:'Wealth & debt',business:'Business'};
  ...
  function accounts() {
    ...
    const connectionRows=activeFeeds.map(c=>`<div class="tr-row"><div><strong>${esc(c.institution_name||'Connected bank feed')}</strong><div class="tr-subtle">Connected · ${esc(title(c.status))}${c.synced_at?' · Last synced '+date(c.synced_at):' · Waiting for its first sync'}</div></div><div class="tr-small-actions">${btn('Sync','sync',`data-id="${c.id}"`)}${btn('Reconnect','reconnect',`data-id="${c.id}"`)}${btn('Disconnect','disconnect',`data-id="${c.id}"`)}</div></div>`).join('');
  ...
  async function connect(connectionId=null) {
    ...
    try {
      ...
      handler=window.Plaid.create({token:token.link_token,onSuccess:async (publicToken,metadata)=>{try{if(connectionId)await api(endpoint(`/banks/${connectionId}/sync`),'POST',{institution_name:metadata?.institution?.name||null});else await api(endpoint('/banks/exchange'),'POST',{public_token:publicToken,institution_name:metadata?.institution?.name||null});notice('Bank connected. History is being synchronized.');await load();}catch(e){notice(e.message);}finally{finish();}},onExit:finish});
      handler.open();
  ...
  root.addEventListener('click',async e=>{
    ...
    try {
      if(['overview','accounts','history','budget','bills','assets','business','medical'].includes(a)){tab=a;filter={};render();}
      ...
      else if(a==='account-activity'){const requestedSpace=active;const rows=await api(endpoint('/evidence'),'POST',{account_id:id});if(requestedSpace!==active)return;evidenceRows=rows;filter={};tab='transactions';render();}
        ...
        dialog(`Review ${suggestion.count} ${suggestion.merchant} purchases`, `<p class="tr-subtle">${esc(suggestion.evidence)}</p><div class="tr-row"><span>Category</span><strong>${esc(title(suggestion.category))}</strong></div><div class="tr-row"><span>Purchases</span><strong>${suggestion.count} · ${precise(suggestion.amount_cents)}</strong></div><p class="tr-subtle">This applies the shown category to these exact purchases only. It does not create a rule for future purchases.</p>`, async()=>{await api(endpoint('/transactions/bulk-classify',sourceId),'POST',{transactions:suggestion.transactions.map(t=>({id:t.id,version:t.version})),category:suggestion.category,flow:suggestion.flow,face_punched:suggestion.face_punched,bullshit_spending:suggestion.bullshit_spending});notice(`${suggestion.count} transactions reviewed.`);},'Apply to these purchases');
      }
