...
function boot() {
  ...
  function accounts() {
    ...
    return `${panel('Your net worth',`<div class="tr-history-intro"><div><div class="tr-number">${money(s.net_worth_cents)}</div><p class="tr-subtle">${s.net_worth_complete?'Based on recorded assets and liabilities':'Partial picture · missing or unverified coverage'}</p></div>${btn('Manage assets & debt','assets')}</div>${data.cash_history.length?'<h3>Cash balance history</h3>'+plot('tr-cash-history','Imported cash balance history','large')+'<p class="tr-subtle">Cash accounts in the balance export only. Select a date to inspect contributing balances.</p>':data.net_worth_history.length?plot('tr-account-history','Observed net worth history','large'):empty('Balance history begins with observations. Importing transactions does not invent historical account balances.')}`)}<div class="tr-grid">${panel('Your accounts',groups.map(([kind,label,color])=>{const rows=data.accounts.filter(a=>a.kind===kind);return rows.length?`<h3>${label} <span class="tr-subtle">${money(rows.reduce((v,a)=>v+(a.balance_cents??0),0))}</span></h3>${rows.map(a=>`<div class="tr-row"><div class="tr-account-name"><span class="tr-account-icon" style="color:${color}">${kind==='cash'?'$':kind==='credit'?'▤':'◈'}</span><div><strong>${esc(a.name)}</strong><div class="tr-source">Observed ${date(a.observed_at)} · History ${a.history_start?a.history_start.slice(0,10):'not imported'}</div></div></div><div><strong>${precise(a.balance_cents)}</strong> ${btn('Activity','account-activity',`data-id="${a.id}"`)}</div></div>`).join('')}`:'';}).join('')||empty('Add your accounts to see your whole financial picture.'),btn('Add account','account'))}${panel('Balance sheet',`<h3>Assets <span class="tr-positive">${money(s.assets_cents)}</span></h3><div class="tr-allocation"><i style="width:${s.assets_cents?Math.max(0,s.cash_cents/s.assets_cents*100):0}%;background:#5b9caa"></i><i style="flex:1;background:#799774"></i></div><div class="tr-row">Liquid cash<strong>${money(s.cash_cents)}</strong></div><div class="tr-row">Other recorded assets<strong>${money(s.assets_cents-s.cash_cents)}</strong></div><h3>Liabilities ${money(s.liabilities_cents)}</h3><p class="tr-subtle">Credit cards, loans, and unpaid medical bills. Missing property, investments, or debts make this a partial picture.</p>${btn('Review coverage','connections')}`)}</div>`;
  }
