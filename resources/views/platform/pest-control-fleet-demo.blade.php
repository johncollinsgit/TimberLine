...
<head>
    ...
    <style>
        ...
        .gs-demo__login { display:inline-flex; margin-top:20px; padding:12px 16px; border:0; border-radius:999px; color:#fffdf7; background:#173e3b; cursor:pointer; font:inherit; font-size:14px; font-weight:800; text-decoration:none; }
    ...
    <main class="gs-demo__wrap">
        ...
        <section class="gs-demo__hero">
            ...
            <div class="gs-demo__grid">
                ...
                <aside class="gs-demo__access"><p class="gs-demo__eyebrow">Demo workspace access</p><h2>Try the fictional workspace</h2><div class="gs-demo__credential"><small>Login</small><code>{{ $demoEmail }}</code></div><div class="gs-demo__credential"><small>Password</small><code>{{ $demoPassword }}</code></div><form method="POST" action="{{ route('platform.pest-control-fleet-demo.login') }}">@csrf<button class="gs-demo__login" type="submit">Enter the demo workspace</button></form><a class="gs-demo__login" style="margin-top:10px;background:#a45338" href="{{ route('platform.pest-control-website') }}">See the customer website</a><p class="gs-demo__note">This opens the isolated fictional workspace directly. Never enter real customer, employee, vehicle, or location data.</p></aside>
            </div>
        </section>
