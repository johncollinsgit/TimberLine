<x-layouts::app :title="'Automations'">
    <div class="min-h-full bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.10),transparent_32%),linear-gradient(180deg,#fafaf9,#f5f5f4)]">
        <div class="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-9">
            <header class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    <div class="mb-3 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-emerald-800">
                        <span class="h-2 w-2 rounded-full bg-emerald-500"></span> First-party automation
                    </div>
                    <h1 class="text-3xl font-black tracking-tight text-zinc-950 sm:text-4xl">Make the busywork run itself.</h1>
                    <p class="mt-2 max-w-2xl text-sm leading-6 text-zinc-600 sm:text-base">Connect the tools your team already uses, test every step, and see exactly what happened on every run.</p>
                </div>
            ...
            <form method="GET" class="grid gap-3 rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm sm:grid-cols-[1fr,180px,auto]">
            ...
            @if($workflows->isEmpty())
                <section class="overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-xl shadow-zinc-900/5">
                    <div class="grid lg:grid-cols-[1.1fr,.9fr]">
            ...
            @else
                <section class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
                    <div class="hidden grid-cols-[minmax(0,1fr),100px,150px,120px,110px,44px] gap-4 border-b border-zinc-200 bg-zinc-50 px-5 py-3 text-[11px] font-black uppercase tracking-wider text-zinc-500 md:grid">
                    ...
                    @foreach($workflows as $workflow)
                        ...
                        <a href="{{ route('workflows.show', $workflow) }}" wire:navigate class="grid gap-4 border-b border-zinc-100 px-5 py-5 transition last:border-0 hover:bg-emerald-50/40 md:grid-cols-[minmax(0,1fr),100px,150px,120px,110px,44px] md:items-center">
