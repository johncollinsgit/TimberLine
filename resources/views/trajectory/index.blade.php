    ...
    <main id="trajectory" class="tr-app" data-spaces="{{ json_encode($spaces->map->only(['id','name','kind'])->values()) }}" data-invite="{{ request('invite','') }}">
        <aside class="tr-sidebar">
            ...
            <nav class="tr-tabs" aria-label="Trajectory sections">
                @foreach(['overview'=>['↗','Trajectory'],'accounts'=>['▤','Accounts'],'transactions'=>['⇄','Transactions'],'history'=>['◷','Cash flow & history'],'budget'=>['▦','Budget'],'bills'=>['▣','Bills & goals'],'medical'=>['♡','Medical sharing'],'assets'=>['◈','Wealth & debt'],'business'=>['▥','Business'],'connections'=>['⚙','Connections']] as $key=>$item)
        ...
        <header class="tr-header"><div><p class="tr-eyebrow">MAKE ROOM FOR WHAT MATTERS</p><h1 id="tr-page-title">Your trajectory</h1><p id="tr-page-subtitle">A clearer picture of today. A better plan for tomorrow.</p></div><div class="tr-actions"><label class="sr-only" for="tr-space">Finance space</label><select id="tr-space"></select><button id="tr-add" class="tr-primary">＋ Add to your plan</button></div></header>
