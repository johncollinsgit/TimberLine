            ...
            <div class="flex flex-wrap items-end justify-between gap-3">
                <form method="GET" action="{{ route('marketing.groups') }}" class="grid gap-3 md:grid-cols-6">
                    ...
                    <div class="md:col-span-2">
                        <label class="text-xs uppercase tracking-[0.2em] text-white/55">Internal</label>
                        <select name="internal" class="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white">
                    ...
                    <div class="md:col-span-6">
                        <button type="submit" class="inline-flex rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm font-semibold text-white/85">
                            Apply Filters
                        </button>
                    </div>
                </form>

                <a href="{{ route('marketing.groups.create') }}" wire:navigate class="inline-flex rounded-full border border-emerald-300/35 bg-emerald-500/20 px-4 py-2 text-sm font-semibold text-emerald-50">
                    Create Group
                </a>
            </div>
        </section>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-white/10">
                        @forelse($groups as $group)
                            <tr>
                                <td class="px-4 py-3 text-white font-semibold">{{ $group->name }}</td>
                                <td class="px-4 py-3">
                                    <span class="inline-flex rounded-full px-2 py-0.5 text-[11px] {{ $group->is_internal ? 'bg-amber-500/20 text-amber-100' : 'bg-white/10 text-white/70' }}">
                                        {{ $group->is_internal ? 'Internal' : 'Standard' }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-white/80">{{ (int) $group->members_count }}</td>
                                <td class="px-4 py-3 text-white/70">{{ \Illuminate\Support\Str::limit((string) $group->description, 110) ?: '—' }}</td>
                                <td class="px-4 py-3 text-white/60">{{ optional($group->updated_at)->format('Y-m-d H:i') }}</td>
                                <td class="px-4 py-3 text-right">
                                    <div class="inline-flex gap-2">
                                        ...
                                        <a href="{{ route('marketing.groups.edit', $group) }}" wire:navigate class="inline-flex rounded-full border border-white/15 bg-white/5 px-3 py-1 text-xs font-semibold text-white/70">Edit</a>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-8 text-center text-white/55">No groups found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
