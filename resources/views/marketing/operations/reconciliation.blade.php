    ...
    <div class="mx-auto w-full max-w-[1800px] px-3 py-4 sm:px-4 sm:py-6 md:px-6 space-y-6 min-w-0">
        ...
        <section class="grid gap-4 md:grid-cols-3">
            <article class="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
                <div class="text-xs uppercase tracking-[0.2em] text-zinc-500">Open Issues</div>
                <div class="mt-2 text-2xl font-semibold text-zinc-950">{{ number_format((int) $openIssueCount) }}</div>
            </article>
            <article class="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
                <div class="text-xs uppercase tracking-[0.2em] text-zinc-500">Issued Codes (Unreconciled)</div>
                <div class="mt-2 text-2xl font-semibold text-zinc-950">{{ number_format((int) $issuedCodeCount) }}</div>
            </article>
            <article class="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
                <div class="text-xs uppercase tracking-[0.2em] text-zinc-500">Redeemed Today</div>
                <div class="mt-2 text-2xl font-semibold text-zinc-950">{{ number_format((int) $reconciledToday) }}</div>
            </article>
        </section>
        ...
        <section class="rounded-3xl border border-zinc-200 bg-zinc-50 p-5 sm:p-6 space-y-4">
            ...
            <form method="GET" action="{{ route('marketing.operations.reconciliation') }}" class="grid gap-3 md:grid-cols-12">
                ...
                <div class="md:col-span-3">
                    <label class="text-xs uppercase tracking-[0.2em] text-zinc-500">Issue Type</label>
                    <select name="issue_type" class="mt-1 w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm text-zinc-950">
                ...
                <div class="md:col-span-2">
                    <label class="text-xs uppercase tracking-[0.2em] text-zinc-500">Platform</label>
                    <select name="platform" class="mt-1 w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm text-zinc-950">
                ...
                <div class="md:col-span-3">
                    ...
                    <input type="text" name="search" value="{{ $search }}" class="mt-1 w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm text-zinc-950" placeholder="event type, endpoint, source id">
                </div>
        ...
        <section class="rounded-3xl border border-zinc-200 bg-zinc-50 p-5 sm:p-6 space-y-4">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <h2 class="text-lg font-semibold text-zinc-950">Unresolved Storefront/Public Issues</h2>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-zinc-200">
                        @forelse($events as $event)
                            <tr>
                                <td class="px-4 py-3 text-zinc-600">{{ optional($event->occurred_at ?: $event->created_at)->format('Y-m-d H:i') ?: '—' }}</td>
                                <td class="px-4 py-3 text-zinc-700">
                                    {{ $event->event_type }}
                                    <div class="text-xs text-zinc-500">{{ $event->endpoint ?: '—' }}</div>
                                </td>
                                <td class="px-4 py-3 text-zinc-600">
                                    {{ $event->issue_type ?: '—' }}
                                    <div class="text-xs text-zinc-500">{{ $event->source_type ?: '—' }}{{ $event->source_id ? (':' . $event->source_id) : '' }}</div>
                                </td>
                                <td class="px-4 py-3 text-zinc-600">
                                    @if($event->profile)
                                ...
                                <td class="px-4 py-3 text-zinc-500">
                                    @if($event->redemption)
                                        ...
                                        <div class="text-xs text-zinc-500">Platform: {{ strtoupper((string) ($event->redemption->platform ?: 'n/a')) }}</div>
                                    @else
                                ...
                                <td class="px-4 py-3 text-zinc-600">
                                    ...
                                    <div class="text-xs text-zinc-500">{{ strtoupper((string) $event->resolution_status) }}</div>
                                </td>
                                <td class="px-4 py-3 text-right">
                                    <div class="flex flex-wrap justify-end gap-2">
                                        ...
                                        <form method="POST" action="{{ route('marketing.operations.reconciliation.issues.resolve', $event) }}">
                                            ...
                                            <button type="submit" class="inline-flex rounded-full border border-zinc-300 bg-zinc-50 px-3 py-1 text-xs font-semibold text-zinc-700">
                                                Ignore
                                            </button>
                                        </form>
                                    </div>
                        ...
                        @empty
                            <tr><td colspan="7" class="px-4 py-6 text-center text-zinc-500">No issues found for current filters.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <section class="rounded-3xl border border-zinc-200 bg-zinc-50 p-5 sm:p-6 space-y-4">
            <h2 class="text-lg font-semibold text-zinc-950">Issued Reward Codes Pending Reconciliation</h2>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-zinc-200">
                        @forelse($issuedRedemptions as $redemption)
                            <tr>
                                <td class="px-4 py-3 text-zinc-600">{{ optional($redemption->issued_at ?: $redemption->created_at)->format('Y-m-d H:i') ?: '—' }}</td>
                                <td class="px-4 py-3 text-zinc-700 font-mono">{{ $redemption->redemption_code }}</td>
                                <td class="px-4 py-3 text-zinc-600">{{ $redemption->reward?->name ?: ('Reward #' . $redemption->reward_id) }}</td>
                                <td class="px-4 py-3 text-zinc-600">
                                ...
                                <td class="px-4 py-3 text-right">
                                    <form method="POST" action="{{ route('marketing.operations.reconciliation.redemptions.mark-redeemed', $redemption) }}" class="inline-flex items-center gap-2">
                                        ...
                                        <input type="text" name="external_order_id" placeholder="Order ID" class="w-28 rounded-lg border border-zinc-200 bg-zinc-50 px-2 py-1 text-xs text-zinc-950">
                                        <button type="submit" class="inline-flex rounded-full border border-zinc-300 bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-900">
                                            Mark Redeemed
                                        </button>
                                    </form>
                        ...
                        @empty
                            <tr><td colspan="6" class="px-4 py-6 text-center text-zinc-500">No issued codes pending reconciliation.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
