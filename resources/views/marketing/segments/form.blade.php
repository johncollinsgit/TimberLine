<x-layouts::app :title="$mode === 'create' ? 'Create Segment' : 'Edit Segment'">
            ...
            <form method="POST" action="{{ $mode === 'create' ? route('marketing.segments.store') : route('marketing.segments.update', $segment) }}" class="space-y-5">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $segment->name) }}" required class="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white" />
                    </div>
                    <div>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        <label class="text-xs uppercase tracking-[0.2em] text-white/55">Channel Scope</label>
                        <select name="channel_scope" class="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white">
                    ...
                    <div>
                        <label class="text-xs uppercase tracking-[0.2em] text-white/55">Rule Logic</label>
                        <select name="rule_logic" class="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white">
                            @php
                ...
                <div>
                    ...
                    <textarea name="description" rows="2" class="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white">{{ old('description', $segment->description) }}</textarea>
                </div>
                        ...
                        @foreach($conditionRows as $idx => $row)
                            <div class="grid gap-2 md:grid-cols-12 segment-condition-row">
                                <div class="md:col-span-4">
                                    <input type="text" name="conditions[{{ $idx }}][field]" value="{{ $row['field'] ?? '' }}" placeholder="field" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white" />
                                </div>
                                <div class="md:col-span-3">
                                    <select name="conditions[{{ $idx }}][operator]" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white">
                                ...
                                <div class="md:col-span-4">
                                    <input type="text" name="conditions[{{ $idx }}][value]" value="{{ is_scalar($row['value'] ?? null) ? (string) $row['value'] : '' }}" placeholder="value" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white" />
                                </div>
                                <div class="md:col-span-1 flex items-center">
                                    <button type="button" class="segment-remove-condition rounded-lg border border-white/15 bg-white/5 px-2 py-1 text-xs text-white/80">Remove</button>
                                </div>
                            </div>
                        @endforeach
                    </div>

                    <button type="button" id="segment-add-condition" class="mt-3 inline-flex rounded-full border border-white/15 bg-white/5 px-3 py-1.5 text-xs text-white/80">
                        Add Condition
                    </button>
                </div>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('marketing.segments') }}" wire:navigate class="inline-flex rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm font-semibold text-white/85">Back</a>
                </div>
            </form>
        </section>
    </div>
    ...
    <script>
        (() => {
            ...
            const rowHtml = (idx) => `
                <div class="grid gap-2 md:grid-cols-12 segment-condition-row">
                    <div class="md:col-span-4"><input type="text" name="conditions[${idx}][field]" placeholder="field" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white" /></div>
                    <div class="md:col-span-3">
                        <select name="conditions[${idx}][operator]" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white">
                    ...
                    <div class="md:col-span-4"><input type="text" name="conditions[${idx}][value]" placeholder="value" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white" /></div>
                    <div class="md:col-span-1 flex items-center"><button type="button" class="segment-remove-condition rounded-lg border border-white/15 bg-white/5 px-2 py-1 text-xs text-white/80">Remove</button></div>
                </div>
            `;
