@php
    ...
    $paid = $agreement->billingOrders->where('agreement_version_id', $agreement->current_version_id)->contains('status', 'paid');
...
<h1 style="margin:0 0 16px;font-size:30px;line-height:1.2;letter-spacing:-1px;">{{ $paid ? 'You’re all set.' : ($accepted ? 'Your agreement is on file.' : 'Your next chapter starts here.') }}</h1>
<p style="margin:0 0 24px;font-size:16px;line-height:1.7;color:#526164;">{{ $agreement->tenant->name }}, your Everbranch agreement is ready. {{ $paid ? 'Your payment is confirmed. Your private link brings together your accepted agreement and payment receipt.' : 'Your private link brings together the scope, pricing, and next steps in one place.' }}</p>
...
<table role="presentation" cellspacing="0" cellpadding="0" style="margin:28px 0 18px;"><tr><td bgcolor="#123c43" style="border-radius:7px;"><a href="{{ $proposalUrl }}" style="display:inline-block;padding:16px 24px;color:#ffffff;font-size:15px;font-weight:700;text-decoration:none;">{{ $paid || $accepted ? 'View your agreement' : 'Review your agreement' }} &rarr;</a></td></tr></table>
<p style="margin:0;font-size:13px;line-height:1.7;color:#526164;">No password needed. {{ $paid ? 'Open your agreement to view the payment confirmation or download your permanent copy.' : ($accepted ? 'Open your agreement to view its details and any remaining next steps.' : 'Review and accept when you’re ready, then continue to secure payment if applicable.') }}</p>
