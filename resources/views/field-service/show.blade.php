        ...
        <div class="fb-workflow-shell fb-workflow-shell--wide field-service-job-shell">
                ...
                <main class="space-y-6">
                    <section class="fb-panel">
                        ...
                        <div class="fb-panel-body grid gap-3 md:grid-cols-2">
                            ...
                            @if($canManage)
                                <details class="rounded-lg border border-zinc-200 p-3 md:col-span-2">
            ...
            <section class="fb-panel">
                ...
                <div class="fb-panel-body space-y-3">
                    <form method="POST" action="{{ route('field-service.notes.store', $job) }}" enctype="multipart/form-data">@csrf<textarea name="body" rows="3" class="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm" placeholder="Share an update or @mention a teammate"></textarea><div class="mt-3 flex flex-wrap items-center gap-3"><label class="fb-btn fb-btn-secondary cursor-pointer"><span>Take or add photos</span><input name="attachments[]" type="file" accept="image/*" capture="environment" multiple class="sr-only"></label><label class="fb-btn fb-btn-secondary cursor-pointer"><span>Add files</span><input name="attachments[]" type="file" accept="image/*,application/pdf,text/plain,text/csv,.doc,.docx,.xls,.xlsx" multiple class="sr-only"></label><span class="text-xs text-zinc-500">Photos, PDFs, documents, spreadsheets, and text files · up to 25 MB each</span></div><button class="fb-btn fb-btn-primary mt-3">Post update</button></form>
                    @forelse($job->notes->sortByDesc('noted_at') as $note)<article class="border-t border-zinc-200 pt-3"><div class="text-sm font-semibold text-zinc-950">{{ $note->createdBy?->name ?? 'Team update' }} <span class="font-normal text-zinc-500">{{ optional($note->noted_at)->diffForHumans() }}</span></div><div class="mt-1 whitespace-pre-wrap text-sm text-zinc-700">{{ $note->body }}</div>@if($updateAttachmentsByNote->has((int) $note->id))<div class="mt-3 flex flex-wrap gap-3">@foreach($updateAttachmentsByNote->get((int) $note->id) as $asset)@if(str_starts_with((string) $asset->mime_type, 'image/'))<a href="{{ route('documents.preview', [$tenant, $asset]) }}" target="_blank" class="group relative h-40 w-56 overflow-hidden rounded-xl border border-zinc-200 bg-zinc-100"><img src="{{ route('documents.preview', [$tenant, $asset]) }}" alt="{{ $asset->caption ?: $asset->file_name }}" class="h-full w-full object-cover transition duration-200 group-hover:scale-[1.03]"><span class="absolute inset-x-0 bottom-0 truncate bg-zinc-950/65 px-2 py-1.5 text-xs font-semibold text-white">{{ $asset->file_name }}</span></a>@else<a href="{{ route('documents.download', [$tenant, $asset]) }}" class="inline-flex min-h-10 max-w-full items-center rounded-lg border border-zinc-200 bg-zinc-50 px-3 text-sm font-semibold text-emerald-900 hover:border-emerald-300"><span class="truncate">File · {{ $asset->file_name }}</span></a>@endif @endforeach</div>@endif</article>@empty<p class="text-sm text-zinc-600">No updates yet.</p>@endforelse
                </div>
            </section>
        </div>
    </flux:main>
