@php
    ...
    $updateAssets = $job->assets->keyBy('id');
@endphp
        ...
        <div class="fb-workflow-shell">
            ...
            <div class="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,.8fr)]">
                <main class="space-y-6">
                    ...
                    <section class="fb-panel">
                        ...
                        <div class="fb-panel-body space-y-3">
                            <form method="POST" action="{{ route('field-service.notes.store', $job) }}" enctype="multipart/form-data" class="space-y-2">@csrf<textarea name="body" rows="3" class="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm" placeholder="Share an update or @mention a teammate"></textarea><label class="block text-xs font-semibold text-zinc-600">Photos or files<input name="attachments[]" type="file" multiple accept="image/*,application/pdf,.pdf" class="mt-1 block w-full text-sm"></label><button class="fb-btn fb-btn-primary">Save update</button></form>
                            @forelse($job->notes->sortByDesc('noted_at') as $note)
                                @php($attachmentIds = collect((array) data_get($note->metadata, 'attachment_asset_ids', []))->filter(fn ($id) => is_numeric($id))->map(fn ($id) => (int) $id))
                                <article class="border-t border-zinc-200 pt-3">
                                    <div class="flex items-start justify-between gap-3"><div class="text-sm font-semibold text-zinc-950">{{ $note->createdBy?->name ?? 'Team update' }} <span class="font-normal text-zinc-500">{{ optional($note->noted_at)->diffForHumans() }}</span></div>@if($canManage)<form method="POST" action="{{ route('field-service.notes.destroy', [$job, $note]) }}" onsubmit="return confirm('Do you want to delete this update?')">@csrf @method('DELETE')<button class="text-xs font-semibold text-rose-700">Delete</button></form>@endif</div>
                                    @if($note->body !== '')<div class="mt-1 whitespace-pre-wrap text-sm text-zinc-700">{{ $note->body }}</div>@endif
                                    @if($attachmentIds->isNotEmpty())<div class="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3">@foreach($attachmentIds as $assetId)@if($asset = $updateAssets->get($assetId))<a href="{{ route('documents.download', [$tenant, $asset]) }}" class="rounded-lg border border-zinc-200 p-2 text-xs font-semibold text-emerald-900">{{ $asset->file_name }}</a>@endif @endforeach</div>@endif
                                </article>
                            @empty<p class="text-sm text-zinc-600">No updates yet.</p>@endforelse
                        </div>
                    </section>
                </main>
                ...
                <aside class="space-y-6">
                    ...
                    <section class="fb-panel"><div class="fb-panel-head"><div class="fb-panel-title">Files</div></div><div class="fb-panel-body space-y-4"><div><div class="mb-2 text-xs font-semibold uppercase tracking-wide text-zinc-500">Drawings & PDFs</div><div class="space-y-2">@forelse($job->assets->filter(fn ($asset) => $asset->mime_type === 'application/pdf') as $asset)<div class="flex min-h-11 items-center gap-2 rounded-lg border border-zinc-200 px-3 text-sm font-semibold text-emerald-900"><a href="{{ route('documents.download', [$tenant, $asset]) }}" class="min-w-0 flex-1 truncate">{{ $asset->file_name }}</a>@if($canManage)<form method="POST" action="{{ route('field-service.assets.destroy', [$job, $asset]) }}" onsubmit="return confirm('Do you want to delete this PDF?')">@csrf @method('DELETE')<button class="text-xs font-semibold text-rose-700">Delete</button></form>@endif</div>@empty<p class="text-sm text-zinc-600">No drawings or PDFs yet. Add one from Files in the Everbranch app.</p>@endforelse</div></div><div><div class="mb-2 text-xs font-semibold uppercase tracking-wide text-zinc-500">Photos</div><div class="space-y-2">@forelse($job->assets->filter(fn ($asset) => str_starts_with((string) $asset->mime_type, 'image/')) as $asset)<div class="flex min-h-11 items-center gap-2 rounded-lg border border-zinc-200 px-3 text-sm font-semibold text-emerald-900"><a href="{{ route('documents.download', [$tenant, $asset]) }}" class="min-w-0 flex-1 truncate">{{ $asset->caption ?: $asset->file_name }}</a>@if($canManage)<form method="POST" action="{{ route('field-service.assets.destroy', [$job, $asset]) }}" onsubmit="return confirm('Do you want to delete this photo?')">@csrf @method('DELETE')<button class="text-xs font-semibold text-rose-700">Delete</button></form>@endif</div>@empty<p class="text-sm text-zinc-600">No photos yet. Add them from the Everbranch app.</p>@endforelse</div></div></div></section>
                </aside>
            </div>
        </div>
    </flux:main>
