...
<form method="post" action="{{ $editing ? route('landlord.invoices.update', [$tenant, $invoice]) : route('landlord.invoices.store', $tenant) }}" class="mx-auto max-w-4xl space-y-6 rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">@csrf @if($editing)@method('put')@endif
    ...
    <section class="rounded-xl border border-zinc-200 p-4"><div class="flex items-start justify-between gap-3"><div><h2 class="font-semibold text-zinc-950">Send to workspace client</h2><p class="mt-1 text-sm text-zinc-600">This contact is saved for the next invoice.</p></div>@if($billingProfileComplete)<span class="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-800">Saved billing profile</span>@else<span class="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-800">One-time setup</span>@endif</div><div class="mt-4 grid gap-4 sm:grid-cols-3"><label class="text-sm font-medium">Workspace client name<input name="customer_name" required maxlength="190" value="{{ old('customer_name', $invoice->customer_name) }}" class="mt-1 block w-full rounded-lg border-zinc-300"></label><label class="text-sm font-medium">Invoice email<input name="customer_email" type="email" required value="{{ old('customer_email', $invoice->customer_email) }}" class="mt-1 block w-full rounded-lg border-zinc-300"></label><label class="text-sm font-medium">Phone <span class="font-normal text-zinc-500">(saved only)</span><input name="customer_phone" type="tel" maxlength="40" autocomplete="tel" placeholder="(864) 555-0100" value="{{ old('customer_phone', $invoice->customer_phone) }}" class="mt-1 block w-full rounded-lg border-zinc-300"><span class="mt-1 block text-xs font-normal text-zinc-500">Billing texts require separate consent.</span></label></div></section>
    <details class="rounded-xl border border-zinc-200 p-4" @if(! $billingProfileComplete || $errors->has('billing_address.*')) open @endif><summary class="cursor-pointer font-semibold text-zinc-900">Billing address <span class="font-normal text-zinc-500">{{ $billingProfileComplete ? '— edit saved details' : '— needed once before sending' }}</span></summary><div class="mt-4 grid gap-4 sm:grid-cols-2"><label class="text-sm font-medium">Address<input name="billing_address[line1]" required value="{{ data_get($address, 'line1') }}" class="mt-1 block w-full rounded-lg border-zinc-300"></label><label class="text-sm font-medium">Address line 2<input name="billing_address[line2]" value="{{ data_get($address, 'line2') }}" class="mt-1 block w-full rounded-lg border-zinc-300"></label><label class="text-sm font-medium">City<input name="billing_address[city]" required value="{{ data_get($address, 'city') }}" class="mt-1 block w-full rounded-lg border-zinc-300"></label><label class="text-sm font-medium">State<input name="billing_address[state]" required value="{{ data_get($address, 'state') }}" class="mt-1 block w-full rounded-lg border-zinc-300"></label><label class="text-sm font-medium">Postal code<input name="billing_address[postal_code]" required value="{{ data_get($address, 'postal_code') }}" class="mt-1 block w-full rounded-lg border-zinc-300"></label><label class="text-sm font-medium">Country code<input name="billing_address[country]" required maxlength="2" value="{{ data_get($address, 'country', 'US') }}" class="mt-1 block w-full rounded-lg border-zinc-300"></label></div></details>
        ...
        <div id="invoice-lines" class="space-y-3">
            @foreach($storedLines as $index => $line)
                <div class="grid gap-3 rounded-xl border border-zinc-200 p-4 sm:grid-cols-12" data-invoice-line>
                    ...
                    <label class="text-sm sm:col-span-2">Unit price ($)<input data-invoice-amount name="lines[{{ $index }}][unit_amount]" required inputmode="decimal" value="{{ data_get($line, 'unit_amount', isset($line['unit_amount_cents']) ? number_format($line['unit_amount_cents'] / 100, 2, '.', '') : '') }}" class="mt-1 block w-full rounded-lg border-zinc-300"></label>
                </div>
            @endforeach
        </div>
    </div>
    <div class="flex items-center justify-between rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950"><span>
...
<script>
    ...
    const updateTotal = () => {
        let total = 0;
        document.querySelectorAll('[data-invoice-line]').forEach((line) => {
            const quantity = Number(line.querySelector('[name$="[quantity]"]')?.value || 0);
            const amount = Number(line.querySelector('[data-invoice-amount]')?.value || 0);
            total += quantity * amount;
        });
        document.getElementById('invoice-total').textContent = total.toLocaleString('en-US', { style: 'currency', currency: 'USD' });
    };
    document.getElementById('invoice-lines')?.addEventListener('input', updateTotal);
    updateTotal();
</script>
</x-app-layout>
