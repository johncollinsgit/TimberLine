...
<form method="post" action="{{ $editing ? route('landlord.invoices.update', [$tenant, $invoice]) : route('landlord.invoices.store', $tenant) }}" class="mx-auto max-w-4xl space-y-6 rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">@csrf @if($editing)@method('put')@endif
    ...
    <div class="grid gap-4 sm:grid-cols-3"><label class="text-sm font-medium">Customer name<input name="customer_name" required maxlength="190" value="{{ old('customer_name', $invoice->customer_name) }}" class="mt-1 block w-full rounded-lg border-zinc-300"></label><label class="text-sm font-medium">Billing email<input name="customer_email" type="email" required value="{{ old('customer_email', $invoice->customer_email) }}" class="mt-1 block w-full rounded-lg border-zinc-300"></label><label class="text-sm font-medium">Billing phone <span class="font-normal text-zinc-500">(optional)</span><input name="customer_phone" type="tel" maxlength="40" autocomplete="tel" placeholder="(864) 555-0100" value="{{ old('customer_phone', $invoice->customer_phone) }}" class="mt-1 block w-full rounded-lg border-zinc-300"><span class="mt-1 block text-xs font-normal text-zinc-500">Saving a number does not establish permission to text it.</span></label></div>
