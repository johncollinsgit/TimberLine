    ...
    <div class="space-y-6">
        <section class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
            ...
            <div class="space-y-8 p-6">
                ...
                @php($operator = is_array($operatorSnapshot ?? null) ? $operatorSnapshot : [])
                <section id="operator-command-center" class="space-y-4 scroll-mt-36">
                    <div class="flex flex-wrap items-end justify-between gap-3"><div><p class="text-xs font-semibold uppercase tracking-[.18em] text-emerald-800">Operator command center</p><h3 class="mt-1 text-lg font-semibold text-zinc-950">Revenue, usage, support, and weekly break-even</h3><p class="mt-1 text-sm text-zinc-600">Weekly summary sends Mondays at 8:00 AM Eastern by text and email. Costs are only as complete as the receipt ledger below.</p></div><a href="{{ route('landlord.support-tickets.index') }}" class="rounded-xl bg-zinc-950 px-4 py-2 text-sm font-semibold text-white">Open Tickets ({{ $operator['open_tickets'] ?? 0 }})</a></div>
