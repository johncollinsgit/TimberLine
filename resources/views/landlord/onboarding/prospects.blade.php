        ...
        @foreach ($prospects as $prospect)
            @foreach ($prospect->communications->where('direction', 'outbound')->where('channel', 'email')->where('status', 'draft') as $communication)
                <div x-cloak x-show="composerId === {{ $communication->id }}" x-transition.opacity class="fixed inset-0 z-50 flex items-end justify-center bg-zinc-950/45 p-0 sm:items-center sm:p-5" @keydown.escape.window="composerId = 0">
                    <div @click.outside="composerId = 0" class="flex h-[min(720px,100dvh)] w-full max-w-3xl flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:h-[min(720px,calc(100dvh-2.5rem))] sm:rounded-2xl">
                        ...
                        <footer class="border-t border-zinc-200 px-4 py-3"><div class="flex flex-wrap items-center gap-2"><span class="mr-1 text-xs font-semibold text-zinc-500">Load template:</span>@foreach ($outreachTemplateOptions as $value => $label)<form method="POST" action="{{ route('landlord.onboarding.prospects.communications.template', [$prospect, $communication]) }}" class="inline">@csrf<input type="hidden" name="template" value="{{ $value }}" /><button class="rounded-lg border border-zinc-300 bg-white px-2.5 py-1.5 text-xs font-semibold text-zinc-700 hover:bg-zinc-100">{{ $label }}</button></form>@endforeach</div></footer>
                    </div>
                </div>
            @endforeach
