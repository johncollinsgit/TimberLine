...
@foreach((array) $version->blocks as $block)
    ...
    <section class="{{ $type === 'hero' ? 'hero' : 'block' }}"><div class="shell @if($type === 'contact_form') card @endif">
        ...
        @elseif($type === 'product_grid')
            <h1>{{ data_get($block, 'heading', 'Shop') }}</h1><p class="copy">{{ data_get($block, 'body') }}</p><a class="product-link" href="{{ route('managed-website.store.index') }}"><strong>Browse products and services</strong><span>Shop directly from this website.</span></a>
        @else
