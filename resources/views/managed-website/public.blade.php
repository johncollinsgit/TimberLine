...
@if(data_get($announcement, 'enabled') && data_get($announcement, 'text'))
    ...
    <a class="announcement" {!! $linkAttributes($announcementLink) !!} data-eb-select="announcement">{{ data_get($announcement, 'text') }}</a>
@endif
<header class="site-shell top" data-eb-select="header" tabindex="{{ $isEditorPreview ? '0' : '-1' }}">
    <a class="brand" {!! $linkAttributes($homeLink) !!} data-eb-select="header">{{ data_get($themeSettings, 'theme_name') ?: $tenant->brandProfile?->display_name ?: $tenant->name }}</a>
    <nav class="nav" aria-label="Main navigation">@foreach($navigation as $item)@php($itemLink = $link(data_get($item, 'url')))<a {!! $linkAttributes($itemLink) !!} data-eb-select="navigation" data-eb-navigation-index="{{ $loop->index }}">{{ data_get($item, 'label') }}</a>@endforeach</nav>
    <details class="mobile-nav" data-eb-select="header"><summary>Menu</summary><nav aria-label="Mobile navigation">@foreach($navigation as $item)@php($itemLink = $link(data_get($item, 'url')))<a {!! $linkAttributes($itemLink) !!} data-eb-select="navigation" data-eb-navigation-index="{{ $loop->index }}">{{ data_get($item, 'label') }}</a>@endforeach</nav></details>
</header>
@foreach((array) $version->blocks as $block)
    ...
    @if($type === 'hero')
        ...
        <section class="hero" style="--hero-image:url('{{ data_get($block, 'image_url') }}')" {!! $blockAttributes !!}><div class="site-shell hero__content"><p class="eyebrow">{{ data_get($block, 'label') }}</p><h1 data-eb-field="heading">{{ data_get($block, 'heading') }}</h1>@if(data_get($block, 'body'))<p class="copy" data-eb-field="body">{{ data_get($block, 'body') }}</p>@endif @if(data_get($block, 'cta_url') && data_get($block, 'cta_label'))<a class="button" {!! $linkAttributes($ctaLink) !!} data-eb-select="section" data-eb-block-index="{{ $loop->index }}" data-eb-field="cta">{{ data_get($block, 'cta_label') }}</a>@endif</div></section>
    @elseif($type === 'service_cards' || $type === 'trust_bar')
        <section class="section {{ $type === 'trust_bar' ? 'section--soft' : '' }}" {!! $blockAttributes !!}><div class="site-shell">@if(data_get($block, 'heading'))<p class="eyebrow">{{ data_get($block, 'label') }}</p><h2 data-eb-field="heading">{{ data_get($block, 'heading') }}</h2>@endif @if(data_get($block, 'body'))<p class="copy" data-eb-field="body">{{ data_get($block, 'body') }}</p>@endif <div class="{{ $type === 'trust_bar' ? 'trust' : 'card-grid' }}">@foreach((array) data_get($block, 'items', []) as $item)<article class="{{ $type === 'trust_bar' ? 'trust__item' : 'card' }}" data-eb-select="section" data-eb-block-index="{{ $loop->parent->index }}" data-eb-item-index="{{ $loop->index }}">@if(data_get($item, 'image_url'))<img src="{{ data_get($item, 'image_url') }}" alt="{{ data_get($item, 'image_alt') }}">@endif@if(data_get($item, 'heading'))<h3>{{ data_get($item, 'heading') }}</h3>@endif@if(data_get($item, 'body'))<p>{{ data_get($item, 'body') }}</p>@endif</article>@endforeach</div></div></section>
    @elseif($type === 'image_with_text')
        @php($ctaLink = $link(data_get($block, 'cta_url')))
        <section class="section" {!! $blockAttributes !!}><div class="site-shell split {{ data_get($block, 'image_position') === 'right' ? 'split--reverse' : '' }}"><div class="split__image">@if(data_get($block, 'image_url'))<img src="{{ data_get($block, 'image_url') }}" alt="{{ data_get($block, 'image_alt') }}">@endif</div><div><p class="eyebrow">{{ data_get($block, 'label') }}</p><h2 data-eb-field="heading">{{ data_get($block, 'heading') }}</h2><p class="copy" data-eb-field="body">{{ data_get($block, 'body') }}</p>@if(data_get($block, 'cta_url') && data_get($block, 'cta_label'))<a class="button" {!! $linkAttributes($ctaLink) !!} data-eb-select="section" data-eb-block-index="{{ $loop->index }}" data-eb-field="cta">{{ data_get($block, 'cta_label') }}</a>@endif</div></div></section>
    @elseif($type === 'gallery')
        <section class="section section--soft" {!! $blockAttributes !!}><div class="site-shell"><p class="eyebrow">{{ data_get($block, 'label') }}</p><h2 data-eb-field="heading">{{ data_get($block, 'heading') }}</h2><div class="gallery">@foreach((array) data_get($block, 'items', []) as $item)@if(data_get($item, 'image_url'))<img src="{{ data_get($item, 'image_url') }}" alt="{{ data_get($item, 'image_alt') }}" data-eb-select="section" data-eb-block-index="{{ $loop->parent->index }}" data-eb-item-index="{{ $loop->index }}">@endif@endforeach</div></div></section>
    @elseif($type === 'faq_list')
        <section class="section" {!! $blockAttributes !!}><div class="site-shell"><p class="eyebrow">{{ data_get($block, 'label') }}</p><h2 data-eb-field="heading">{{ data_get($block, 'heading') }}</h2><div class="faq">@foreach((array) data_get($block, 'items', []) as $item)<details data-eb-select="section" data-eb-block-index="{{ $loop->parent->index }}" data-eb-item-index="{{ $loop->index }}"><summary>{{ data_get($item, 'heading') }}</summary><p>{{ data_get($item, 'body') }}</p></details>@endforeach</div></div></section>
    @elseif($type === 'testimonial')
        <section class="section section--soft" {!! $blockAttributes !!}><div class="site-shell"><p class="eyebrow">{{ data_get($block, 'label', 'Customer-first service') }}</p><blockquote class="quote" data-eb-field="body">“{{ data_get($block, 'body') }}”</blockquote>@if(data_get($block, 'heading'))<strong data-eb-field="heading">{{ data_get($block, 'heading') }}</strong>@endif</div></section>
    @elseif($type === 'contact_form')
        <section class="section section--soft" id="contact" {!! $blockAttributes !!}><div class="site-shell contact-card"><p class="eyebrow">{{ data_get($block, 'label', 'Contact') }}</p><h2 data-eb-field="heading">{{ data_get($block, 'heading', 'Contact us') }}</h2>@if(data_get($block, 'body'))<p class="copy" data-eb-field="body">{{ data_get($block, 'body') }}</p>@endif @if(session('website_form_status'))<p role="status">{{ session('website_form_status') }}</p>@endif @if($isDraftPreview)<button class="button" type="button" data-eb-select="section" data-eb-block-index="{{ $loop->index }}">Preview form</button>@else<form method="POST" action="{{ route('managed-website.forms.submit', ['page' => $page]) }}">@csrf<input type="text" name="website" hidden tabindex="-1" autocomplete="off"><label class="field">Name<input required name="name"></label><label class="field">Email<input required type="email" name="email"></label><label class="field">Phone<input name="phone"></label><label class="field">How can we help?<textarea required name="message"></textarea></label><button class="button" type="submit">Send message</button></form>@endif</div></section>
    @else
        @php($ctaLink = $link(data_get($block, 'cta_url')))
        <section class="section {{ $type === 'cta' ? 'section--soft' : '' }}" {!! $blockAttributes !!}><div class="site-shell">@if(data_get($block, 'label'))<p class="eyebrow">{{ data_get($block, 'label') }}</p>@endif @if(data_get($block, 'heading'))<h2 data-eb-field="heading">{{ data_get($block, 'heading') }}</h2>@endif @if(data_get($block, 'body'))<p class="copy" data-eb-field="body">{{ data_get($block, 'body') }}</p>@endif @if(data_get($block, 'image_url'))<img src="{{ data_get($block, 'image_url') }}" alt="{{ data_get($block, 'image_alt') }}" style="border-radius:var(--corner);max-width:100%">@endif @if(data_get($block, 'cta_url') && data_get($block, 'cta_label'))<a class="button" {!! $linkAttributes($ctaLink) !!} data-eb-select="section" data-eb-block-index="{{ $loop->index }}" data-eb-field="cta">{{ data_get($block, 'cta_label') }}</a>@endif</div></section>
    @endif
@endforeach
<footer class="site-footer" data-eb-select="footer" tabindex="{{ $isEditorPreview ? '0' : '-1' }}"><div class="site-shell site-footer__grid"><div><a class="brand" {!! $linkAttributes($homeLink) !!} data-eb-select="footer">{{ data_get($themeSettings, 'theme_name') ?: $tenant->name }}</a><p>{{ data_get($footer, 'tagline') }}</p><p>{{ data_get($footer, 'copyright') ?: '© '.now()->year.' '.(data_get($themeSettings, 'theme_name') ?: $tenant->name) }}</p></div><div>@foreach($navigation as $item)@php($itemLink = $link(data_get($item, 'url')))<a {!! $linkAttributes($itemLink) !!} data-eb-select="navigation" data-eb-navigation-index="{{ $loop->index }}">{{ data_get($item, 'label') }}</a>@endforeach</div></div></footer>
@if($isEditorPreview)
<script>
    (() => {
