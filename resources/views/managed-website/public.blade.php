@php
    ...
    $hasInteractiveProductViewer = collect((array) $version->blocks)
        ->contains(static fn (mixed $block): bool => data_get($block, 'type') === 'interactive_product_viewer');
    $hasProductVideo = collect((array) $version->blocks)
        ->contains(static fn (mixed $block): bool => data_get($block, 'type') === 'product_video');
...
<head>
    ...
    <style>.hero{background:linear-gradient(100deg,color-mix(in srgb,var(--ink) 90%,transparent),color-mix(in srgb,var(--ink) 38%,transparent)),var(--hero-image,var(--ink)) center/cover}</style>
    @if($hasInteractiveProductViewer || $hasProductVideo)
        <style>
            .product-viewer-section{overflow:hidden}.product-viewer{background:radial-gradient(circle at 50% 25%,color-mix(in srgb,var(--soft) 20%,#fff),var(--soft));border:1px solid color-mix(in srgb,var(--ink) 12%,transparent);border-radius:var(--corner);box-shadow:0 24px 60px -44px color-mix(in srgb,var(--ink) 70%,transparent);min-height:clamp(340px,48vw,590px);overflow:hidden;position:relative}.product-viewer__canvas{height:clamp(340px,48vw,590px);position:relative;width:100%}.product-viewer__canvas canvas{display:block;height:100%;inset:0;position:absolute;width:100%}.product-viewer__poster{display:block;height:100%;inset:0;object-fit:cover;position:absolute;transition:opacity .35s ease;width:100%;z-index:1}.product-viewer.is-ready .product-viewer__poster{opacity:0;pointer-events:none}.product-viewer__placeholder{align-items:center;color:color-mix(in srgb,var(--ink) 62%,transparent);display:flex;font-size:.93rem;justify-content:center;min-height:inherit;padding:2rem;text-align:center}.product-viewer__controls{align-items:center;bottom:1rem;display:flex;flex-wrap:wrap;gap:.55rem;left:1rem;position:absolute;right:1rem;z-index:4}.product-viewer__controls .button{box-shadow:0 10px 30px -18px color-mix(in srgb,var(--ink) 80%,transparent);cursor:pointer;margin:0}.product-viewer__controls .button[disabled]{cursor:not-allowed;opacity:.62}.product-viewer__controls .button--secondary{background:#fff;border-color:#fff;color:var(--ink)}.product-viewer__status{background:color-mix(in srgb,var(--ink) 82%,transparent);border-radius:999px;color:#fff;font-size:.75rem;font-weight:700;margin-left:auto;padding:.5rem .7rem}.product-viewer__variants{display:flex;gap:.45rem;left:1rem;position:absolute;top:1rem;z-index:4}.product-viewer__variant{background:color-mix(in srgb,var(--ink) 82%,transparent);border:1px solid color-mix(in srgb,#fff 45%,transparent);border-radius:999px;color:#fff;cursor:pointer;font-size:.72rem;font-weight:800;padding:.48rem .7rem}.product-viewer__variant[aria-pressed=true]{background:#fff;color:var(--ink)}.product-viewer__hotspots{inset:0;pointer-events:none;position:absolute;z-index:3}.product-viewer__hotspot{background:var(--accent);border:2px solid #fff;border-radius:999px;box-shadow:0 8px 20px -10px color-mix(in srgb,var(--ink) 85%,transparent);color:var(--ink);cursor:pointer;font-size:.74rem;font-weight:850;padding:.42rem .64rem;pointer-events:auto;position:absolute;transform:translate(-50%,-50%)}.product-viewer__hotspot:focus-visible{outline:3px solid var(--brand);outline-offset:3px}.product-viewer__hotspot[aria-expanded=true]{background:#fff}.product-viewer__hotspot[aria-expanded=true]::after{background:var(--ink);border-radius:.45rem;color:#fff;content:attr(title);font-size:.72rem;font-weight:650;left:50%;line-height:1.35;max-width:190px;padding:.55rem .65rem;position:absolute;top:calc(100% + .55rem);transform:translateX(-50%);width:max-content;z-index:5}.product-video{border-radius:var(--corner);box-shadow:0 24px 60px -44px color-mix(in srgb,var(--ink) 70%,transparent);display:block;max-width:100%;width:100%}@media(prefers-reduced-motion:reduce){.product-viewer__poster{transition:none}}@media(max-width:760px){.product-viewer{min-height:360px}.product-viewer__canvas{height:360px}.product-viewer__status{flex-basis:100%;margin-left:0;max-width:max-content}.product-viewer__hotspot{font-size:.68rem;padding:.35rem .52rem}}
        </style>
        @vite('resources/js/managed-website/product-viewer.ts')
    @endif
</head>
...
@foreach((array) $version->blocks as $block)
    ...
    @elseif($type === 'product_video')
        @php($videoMedia = app(\App\Services\ManagedWebsite\ManagedWebsiteService::class)->productVideoMedia($site, (array) $block))
        <section class="section" {!! $blockAttributes !!}><div class="site-shell"><p class="eyebrow" data-eb-field="label">{{ data_get($block, 'label') }}</p>@if(data_get($block, 'heading'))<h2 data-eb-field="heading">{{ data_get($block, 'heading') }}</h2>@endif @if(data_get($block, 'body'))<p class="copy" data-eb-field="body">{{ data_get($block, 'body') }}</p>@endif @if(data_get($videoMedia, 'video_url'))<video class="product-video" controls playsinline preload="metadata" @if(data_get($videoMedia, 'poster_url')) poster="{{ data_get($videoMedia, 'poster_url') }}" @endif aria-label="{{ data_get($block, 'video_alt', data_get($block, 'heading', 'Product video')) }}"><source src="{{ data_get($videoMedia, 'video_url') }}" type="video/mp4">Your browser does not support this product video.</video>@endif</div></section>
    @elseif($type === 'interactive_product_viewer')
        @php($ctaLink = $link(data_get($block, 'cta_url')))
        @php($hotspots = array_values((array) data_get($block, 'hotspots', [])))
        @php($viewerMedia = app(\App\Services\ManagedWebsite\ManagedWebsiteService::class)->interactiveProductViewerMedia($site, (array) $block))
        @php($viewerVariants = app(\App\Services\ManagedWebsite\ManagedWebsiteService::class)->interactiveProductViewerVariants($site, (array) $block))
        <section class="section section--soft product-viewer-section" @if(data_get($block, 'id')) id="{{ data_get($block, 'id') }}" @endif {!! $blockAttributes !!}>
            <div class="site-shell split">
                <div>
                    @if(data_get($block, 'label'))<p class="eyebrow" data-eb-field="label">{{ data_get($block, 'label') }}</p>@endif
                    @if(data_get($block, 'heading'))<h2 data-eb-field="heading">{{ data_get($block, 'heading') }}</h2>@endif
                    @if(data_get($block, 'body'))<p class="copy" data-eb-field="body">{{ data_get($block, 'body') }}</p>@endif
                    @if(data_get($block, 'cta_url') && data_get($block, 'cta_label'))<a class="button" {!! $linkAttributes($ctaLink) !!} data-eb-select="section" data-eb-block-index="{{ $loop->index }}" data-eb-field="cta_label">{{ data_get($block, 'cta_label') }}</a>@endif
                </div>
                <div class="product-viewer" data-everbranch-product-viewer data-model-url="{{ data_get($viewerMedia, 'model_url') }}" data-poster-url="{{ data_get($viewerMedia, 'poster_url') }}" data-raise-clip="{{ data_get($block, 'raise_clip', 'raise') }}" data-lower-clip="{{ data_get($block, 'lower_clip', 'lower') }}" data-hotspots='@json($hotspots)' data-product-variants='@json($viewerVariants)' aria-busy="true" aria-label="Interactive product viewer">
                    <div class="product-viewer__canvas" data-product-viewer-canvas>
                        @if(data_get($viewerMedia, 'poster_url'))<img class="product-viewer__poster" src="{{ data_get($viewerMedia, 'poster_url') }}" alt="{{ data_get($block, 'poster_alt', data_get($block, 'heading', 'Product')) }}" loading="lazy">@else<div class="product-viewer__placeholder">A product image will appear here while the interactive model loads.</div>@endif
                    </div>
                    @if(count($viewerVariants) > 1)<div class="product-viewer__variants" role="group" aria-label="Choose a product configuration">@foreach($viewerVariants as $variant)<button class="product-viewer__variant" type="button" data-product-viewer-variant="{{ data_get($variant, 'id') }}" aria-pressed="{{ $loop->first ? 'true' : 'false' }}">{{ data_get($variant, 'label') }}</button>@endforeach</div>@endif
                    <div class="product-viewer__hotspots" data-product-viewer-hotspots aria-label="Product details"></div>
                    <div class="product-viewer__controls" aria-label="Product viewer controls">
                        <button class="button" type="button" data-product-viewer-action="raise" data-eb-field="raise_label" disabled>{{ data_get($block, 'raise_label', 'Raise') }}</button>
                        <button class="button button--secondary" type="button" data-product-viewer-action="lower" data-eb-field="lower_label" disabled>{{ data_get($block, 'lower_label', 'Lower') }}</button>
                        <span class="product-viewer__status" data-product-viewer-status role="status" aria-live="polite">Interactive view loading…</span>
                    </div>
                    <noscript>@if(data_get($viewerMedia, 'poster_url'))<img class="product-viewer__poster" src="{{ data_get($viewerMedia, 'poster_url') }}" alt="{{ data_get($block, 'poster_alt', data_get($block, 'heading', 'Product')) }}">@endif</noscript>
                </div>
            </div>
        </section>
    @else
