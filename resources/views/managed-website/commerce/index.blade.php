<x-layouts::app.sidebar :title="str($screen)->headline()">
    <flux:main>
        <div class="mx-auto max-w-[1440px] space-y-6 pb-10">
            ...
            <header class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                @if($screen === 'products')
                    <div class="flex flex-wrap gap-2">
                        ...
                        @if($shippingEnabled ?? false)<a class="fb-btn fb-btn-secondary" href="{{ route('managed-website.commerce.shipping') }}">Shipping setup</a>@endif
            ...
            @else
                <section class="rounded-2xl border border-zinc-200 bg-zinc-50 p-4"><form class="grid gap-3 sm:grid-cols-4" method="GET" action="{{ route('managed-website.orders.index') }}"><select name="payment" class="rounded-lg border-zinc-300 text-sm"><option value="">All payments</option>@foreach(['pending','paid','failed','partially_refunded','refunded'] as $status)<option value="{{ $status }}" @selected(data_get($filters ?? [], 'payment') === $status)>{{ str($status)->headline() }}</option>@endforeach</select><select name="fulfillment" class="rounded-lg border-zinc-300 text-sm"><option value="">All fulfillment</option>@foreach(['unfulfilled','partial','fulfilled','cancelled'] as $status)<option value="{{ $status }}" @selected(data_get($filters ?? [], 'fulfillment') === $status)>{{ str($status)->headline() }}</option>@endforeach</select><select name="shipment" class="rounded-lg border-zinc-300 text-sm"><option value="">All shipments</option>@foreach(['pending','label_purchased','in_transit','delivered','failure','voided'] as $status)<option value="{{ $status }}" @selected(data_get($filters ?? [], 'shipment') === $status)>{{ str($status)->headline() }}</option>@endforeach</select><button class="fb-btn fb-btn-secondary justify-center" type="submit">Filter orders</button></form></section>
                <section class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm"><div class="flex items-center justify-between border-b border-zinc-200 p-4"><strong class="text-sm">Website orders</strong><span class="text-xs text-zinc-500">Native lane only</span></div><div class="overflow-x-auto"><table class="min-w-full text-left text-sm"><thead class="bg-zinc-50 text-xs uppercase tracking-wide text-zinc-500"><tr><th class="px-5 py-3">Order</th><th class="px-5 py-3">Customer</th><th class="px-5 py-3">Payment</th><th class="px-5 py-3">Fulfillment</th><th class="px-5 py-3 text-right">Total</th><th class="px-5 py-3"></th></tr></thead><tbody class="divide-y divide-zinc-100">@forelse($orders as $order)<tr><td class="px-5 py-4 font-semibold text-zinc-950">{{ $order->number }}<span class="mt-1 block text-xs font-normal text-zinc-500">{{ $order->created_at->format('M j, Y g:i A') }}</span></td><td class="px-5 py-4">{{ data_get($order->customer_snapshot, 'name', '—') }}<span class="mt-1 block text-xs text-zinc-500">{{ data_get($order->customer_snapshot, 'email') }}</span></td><td class="px-5 py-4"><span class="rounded-full px-2 py-1 text-xs font-bold {{ $order->payment_status === 'paid' ? 'bg-emerald-100 text-emerald-900' : 'bg-zinc-100 text-zinc-700' }}">{{ str($order->payment_status)->headline() }}</span></td><td class="px-5 py-4">{{ str($order->fulfillment_status)->headline() }}<span class="mt-1 block text-xs text-zinc-500">{{ str($order->fulfillment_method)->headline() }}@if($order->shipments->first()) · {{ str($order->shipments->first()->status)->headline() }}@endif</span></td><td class="px-5 py-4 text-right font-semibold">${{ number_format($order->total_cents / 100, 2) }}</td><td class="px-5 py-4"><a class="text-sm font-bold text-emerald-800 hover:underline" href="{{ route('managed-website.orders.show', ['order' => $order]) }}">Open</a></td></tr>@empty<tr><td colspan="6" class="px-5 py-14 text-center text-sm text-zinc-500">Paid Website orders appear here after Stripe confirms payment.</td></tr>@endforelse</tbody></table></div></section>
            @endif
        </div>
    </flux:main>
