    ...
    <div class="space-y-6">
        ...
        @if($sectionKey === 'customers')
            <section class="grid gap-4 xl:grid-cols-4">
                ...
                <article class="rounded-[1.7rem] border border-zinc-200 bg-zinc-50 p-5">
                    ...
                    <div class="mt-3 text-4xl font-semibold text-zinc-950">{{ number_format((int) data_get($summary, 'birthdays_today', 0)) }}</div>
                    <p class="mt-2 text-sm text-zinc-600">Birthdays happening today.</p>
                </article>
                <article class="rounded-[1.7rem] border border-zinc-200 bg-zinc-50 p-5">
                    <div class="text-[11px] uppercase tracking-[0.24em] text-zinc-500">Email Ready</div>
                    <div class="mt-3 text-4xl font-semibold text-zinc-950">{{ number_format((int) data_get($summary, 'email_subscribed', 0)) }}</div>
                    <p class="mt-2 text-sm text-zinc-600">Birthday club customers who can receive email.</p>
                </article>
                <article class="rounded-[1.7rem] border border-zinc-200 bg-zinc-50 p-5">
                    <div class="text-[11px] uppercase tracking-[0.24em] text-zinc-500">Shopify Match</div>
                    <div class="mt-3 text-4xl font-semibold text-zinc-950">{{ number_format((int) data_get($summary, 'shopify_matched', 0)) }}</div>
                    <p class="mt-2 text-sm text-zinc-600">Birthday club records already matched to Shopify-linked customers.</p>
                </article>
            </section>

            <section class="grid gap-4 xl:grid-cols-[minmax(0,1.2fr),minmax(360px,0.8fr)]">
                <article class="rounded-[1.8rem] border border-zinc-200 bg-zinc-50 p-5 space-y-4">
                    ...
                    <form method="POST" action="{{ route('birthdays.customers.import.preview') }}" enctype="multipart/form-data" class="flex flex-col gap-3 md:flex-row md:items-end">
                        ...
                        <div class="flex-1">
                            ...
                            <input type="file" name="import_file" accept=".csv,text/csv" class="mt-2 block w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950 file:mr-4 file:rounded-full file:border-0 file:bg-zinc-100 file:px-4 file:py-2 file:text-xs file:font-semibold file:text-zinc-950" />
                            @error('import_file')
                                <div class="mt-2 text-sm text-rose-200">{{ $message }}</div>
                            @enderror
                        </div>
                        <button type="submit" class="inline-flex rounded-full border border-rose-300/30 bg-rose-100 px-5 py-3 text-sm font-semibold text-zinc-950">Preview Import</button>
                    </form>
                </article>

                <article class="rounded-[1.8rem] border border-zinc-200 bg-zinc-50 p-5">
                    <div class="text-[11px] uppercase tracking-[0.24em] text-zinc-500">Filters</div>
                    <h2 class="mt-2 text-lg font-semibold text-zinc-950">Find birthday customers fast</h2>
                    <form method="GET" action="{{ route('birthdays.customers') }}" class="mt-4 grid gap-3 md:grid-cols-2">
                        <input type="text" name="search" value="{{ data_get($filters, 'search') }}" placeholder="Search name, email, phone, source" class="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950 placeholder:text-zinc-500 md:col-span-2" />
                        <select name="month" class="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950">
                        ...
                        <select name="timing" class="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950">
                        ...
                        <select name="subscription" class="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950">
                        ...
                        <div class="md:col-span-2 flex gap-3">
                            <button type="submit" class="inline-flex rounded-full border border-zinc-300 bg-zinc-50 px-4 py-2 text-sm font-semibold text-zinc-950">Apply</button>
                            <a href="{{ route('birthdays.customers') }}" wire:navigate class="inline-flex rounded-full border border-zinc-200 px-4 py-2 text-sm font-semibold text-zinc-600">Reset</a>
                        </div>
                    </form>
                </article>
            </section>
            ...
            @if(!empty($importPreview))
                <section class="rounded-[1.8rem] border border-zinc-200 bg-zinc-50 p-5 space-y-4">
                    <div class="flex items-end justify-between gap-3">
                        <div>
                            <div class="text-[11px] uppercase tracking-[0.24em] text-zinc-500">Preview</div>
                            <h2 class="mt-2 text-lg font-semibold text-zinc-950">{{ $importPreview['file_name'] }}</h2>
                            <p class="mt-2 text-sm text-zinc-600">Adjust the column mapping if needed, then import.</p>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('birthdays.customers.import.run') }}" class="space-y-4">
                        ...
                        <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                            @foreach($importPreview['headers'] as $header)
                                <label class="block rounded-2xl border border-zinc-200 bg-zinc-50 p-3">
                                    <div class="text-xs uppercase tracking-[0.2em] text-zinc-500">{{ $header }}</div>
                                    <select name="mapping[{{ $header }}]" class="mt-3 w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm text-zinc-950">
                        ...
                        <button type="submit" class="inline-flex rounded-full border border-rose-300/30 bg-rose-100 px-5 py-3 text-sm font-semibold text-zinc-950">Import Birthdays</button>
                    </form>
                </section>
            @endif
            ...
            <section class="rounded-[1.8rem] border border-zinc-200 bg-zinc-50 p-5">
                <div class="flex items-end justify-between gap-3">
                    <div>
                        <div class="text-[11px] uppercase tracking-[0.24em] text-zinc-500">Customers</div>
                        <h2 class="mt-2 text-lg font-semibold text-zinc-950">Birthday club customers</h2>
                    </div>
                    <div class="text-sm text-zinc-500">{{ number_format($profiles->total()) }} total</div>
                </div>

                <div class="mt-4 overflow-x-auto rounded-[1.4rem] border border-zinc-200">
                    <table class="min-w-full text-left text-sm text-zinc-700">
                        <thead class="bg-zinc-50 text-xs uppercase tracking-[0.18em] text-zinc-500">
                            <tr>
                        ...
                        <tbody>
                            @forelse($profiles as $birthdayProfile)
                                ...
                                <tr class="border-t border-zinc-200">
                                    <td class="px-4 py-3 align-top">
                                        <div class="font-semibold text-zinc-950">{{ trim(($profile->first_name ?? '').' '.($profile->last_name ?? '')) ?: 'Unknown' }}</div>
                                        <div class="mt-1 text-xs text-zinc-500">{{ $profile->email ?: ($profile->phone ?: 'No email or phone') }}</div>
                                        @if($shopifyLink)
                                            <div class="mt-1 text-[11px] text-zinc-500">Shopify {{ $shopifyLink->source_id }}</div>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 align-top">
                                        <div class="font-medium text-zinc-950">{{ sprintf('%02d/%02d', (int) $birthdayProfile->birth_month, (int) $birthdayProfile->birth_day) }}</div>
                                        <div class="mt-1 text-xs text-zinc-500">{{ $birthdayProfile->birth_year ?: 'Year optional' }}</div>
                                    </td>
                                    <td class="px-4 py-3 align-top">
                                        <div>{{ $birthdayProfile->signup_source ?: '—' }}</div>
                                        <div class="mt-1 text-xs text-zinc-500">Imported {{ optional($birthdayProfile->capture_date)->format('Y-m-d') ?: '—' }}</div>
                                    </td>
                                    <td class="px-4 py-3 align-top">
                                        <div>Email: {{ $birthdayProfile->email_subscribed ? 'Yes' : ($birthdayProfile->email_subscribed === false ? 'No' : '—') }}</div>
                                        <div class="mt-1 text-xs text-zinc-500">SMS: {{ $birthdayProfile->sms_subscribed ? 'Yes' : ($birthdayProfile->sms_subscribed === false ? 'No' : '—') }}</div>
                                    </td>
                                    <td class="px-4 py-3 align-top">
                                        <div class="font-medium text-zinc-950">{{ $issuance?->reward_name ?: 'No reward yet' }}</div>
                                        <div class="mt-1 text-xs text-zinc-500">{{ $issuance?->status ? ucfirst($issuance->status) : '—' }}</div>
                                        @if($issuance?->reward_code)
                                            <div class="mt-1 text-[11px] text-zinc-500">{{ $issuance->reward_code }}</div>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 align-top">
                                    ...
                                    <td class="px-4 py-3 align-top">
                                        <div class="flex flex-wrap gap-2">
                                            <a href="{{ route('marketing.customers.show', $profile) }}" wire:navigate class="inline-flex rounded-full border border-zinc-200 px-3 py-1 text-xs font-semibold text-zinc-700">Open</a>
                                            <form method="POST" action="{{ route('birthdays.customers.issue-reward', $profile) }}">
                                                @csrf
                                                <button type="submit" class="inline-flex rounded-full border border-zinc-200 px-3 py-1 text-xs font-semibold text-zinc-700">Issue</button>
                                            </form>
                                            @if($issuance && $issuance->status === 'issued')
                                                <form method="POST" action="{{ route('birthdays.rewards.activate', $issuance) }}">
                                                    @csrf
                                                    <button type="submit" class="inline-flex rounded-full border border-rose-300/30 bg-rose-100 px-3 py-1 text-xs font-semibold text-zinc-950">Activate</button>
                                                </form>
                                            @endif
                                        </div>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-4 py-6 text-sm text-zinc-500">No birthday customers match the current filters.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                @foreach([
                    ...
                    <article class="rounded-[1.8rem] border border-zinc-200 bg-zinc-50 p-5">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                <div class="text-[11px] uppercase tracking-[0.24em] text-zinc-500">{{ $campaign['title'] }}</div>
                                <h2 class="mt-2 text-lg font-semibold text-zinc-950">{{ $campaign['enabled'] ? 'Enabled' : 'Disabled' }}</h2>
                            </div>
                            <span class="inline-flex rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1 text-xs font-semibold text-zinc-700">Offset {{ $campaign['offset'] }}d</span>
                        </div>
                        <div class="mt-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
                            <div class="text-xs uppercase tracking-[0.18em] text-zinc-500">Preview</div>
                            <div class="mt-2 text-sm font-semibold text-zinc-950">{{ $campaign['preview'] }}</div>
                            <p class="mt-2 text-sm leading-6 text-zinc-600">{{ $campaign['body'] }}</p>
                        </div>
            ...
            <section class="grid gap-4 xl:grid-cols-[minmax(0,0.85fr),minmax(0,1.15fr)]">
                <article class="rounded-[1.8rem] border border-zinc-200 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(0,0,0,0.16))] p-5 shadow-[0_24px_90px_rgba(0,0,0,0.28)]">
                    ...
                    <form method="POST" action="{{ route('birthdays.settings.save') }}" class="mt-4 grid gap-3">
                        ...
                        <label class="block">
                            <span class="text-xs uppercase tracking-[0.18em] text-zinc-500">Reward Name</span>
                            <input type="text" name="reward_name" value="{{ data_get($rewardConfig, 'reward_name') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950" />
                        </label>
                        <div class="grid gap-3 md:grid-cols-2">
                            <label class="block">
                                <span class="text-xs uppercase tracking-[0.18em] text-zinc-500">Reward Type</span>
                                <select name="reward_type" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950">
                                    @foreach(['discount_code' => 'Fixed amount code', 'candle_cash' => 'Rewards', 'free_shipping' => 'Free shipping'] as $value => $label)
                                        <option value="{{ $value }}" @selected(data_get($rewardConfig, 'reward_type') === $value)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </label>
                            <label class="block">
                                <span class="text-xs uppercase tracking-[0.18em] text-zinc-500">Reward Value</span>
                                <input type="number" step="0.01" min="0" name="reward_value" value="{{ data_get($rewardConfig, 'reward_value') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950" />
                            </label>
                        </div>
                        <div class="grid gap-3 md:grid-cols-2">
                            <label class="block">
                                <span class="text-xs uppercase tracking-[0.18em] text-zinc-500">Coupon Prefix</span>
                                <input type="text" name="discount_code_prefix" value="{{ data_get($rewardConfig, 'discount_code_prefix') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950" />
                            </label>
                            <label class="block">
                                <span class="text-xs uppercase tracking-[0.18em] text-zinc-500">Expiration Days</span>
                                <input type="number" min="1" name="claim_window_days_after" value="{{ data_get($rewardConfig, 'claim_window_days_after') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950" />
                            </label>
                        </div>
                        <button type="submit" class="inline-flex rounded-full border border-rose-300/30 bg-rose-100 px-5 py-3 text-sm font-semibold text-zinc-950">Save Reward Rules</button>
                    </form>
                </article>
                                    ...
                                    <tr class="border-t border-zinc-200">
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="inline-flex rounded-full border px-3 py-1 text-xs font-semibold
                                                @if($issuance->status === 'redeemed') border-zinc-300 bg-emerald-100 text-emerald-900
                                                @elseif($issuance->status === 'claimed') border-sky-300/35 bg-sky-100 text-sky-900
                                                @elseif($issuance->status === 'expired') border-amber-300/35 bg-amber-100 text-amber-900
                                                @elseif($issuance->status === 'cancelled') border-zinc-300 bg-zinc-50 text-zinc-500
                                                @else border-zinc-300 bg-zinc-50 text-zinc-700 @endif">
                                        ...
                                        <td class="px-4 py-3 align-top">
                                            <div class="font-mono text-xs text-zinc-950">{{ $issuance->reward_code ?: '—' }}</div>
                                            <div class="mt-2 inline-flex rounded-full border px-2.5 py-1 text-[11px] font-semibold
                                                @if($syncStatus === 'synced') border-zinc-300 bg-emerald-100 text-emerald-900
                                                @elseif($syncStatus === 'failed') border-rose-300/35 bg-rose-100 text-rose-900
                                                @elseif($syncStatus === 'syncing') border-sky-300/35 bg-sky-100 text-sky-900
                                                @else border-zinc-300 bg-zinc-50 text-zinc-600 @endif">
                                                {{ str($syncStatus)->replace('_', ' ')->title() }}
                                            </div>
                                            @if($issuance->shopify_store_key)
                                                <div class="mt-1 text-[11px] text-zinc-500">{{ $issuance->shopify_store_key }}</div>
                                            @endif
                                            @if($issuance->discount_sync_error)
                                                <div class="mt-2 rounded-xl border border-rose-300/20 bg-rose-100 px-3 py-2 text-[11px] leading-5 text-rose-900">{{ $issuance->discount_sync_error }}</div>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex flex-wrap gap-2">
                                                @if(in_array($issuance->status, ['issued', 'claimed'], true) && ! in_array($syncStatus, ['synced', 'not_applicable'], true))
                                                    <form method="POST" action="{{ route('birthdays.rewards.activate', $issuance) }}">
                                                        @csrf
                                                        <button type="submit" class="inline-flex rounded-full border border-zinc-200 px-3 py-1 text-xs font-semibold text-zinc-700">{{ $issuance->status === 'claimed' ? 'Retry Sync' : 'Activate' }}</button>
                                                    </form>
                                                @endif
                                                @foreach(['redeemed' => 'Mark Used', 'expired' => 'Expire', 'cancelled' => 'Cancel'] as $value => $label)
                                                    <form method="POST" action="{{ route('birthdays.rewards.status', $issuance) }}">
                                                        ...
                                                        <button type="submit" class="inline-flex rounded-full border border-zinc-200 px-3 py-1 text-xs font-semibold text-zinc-700">{{ $label }}</button>
                                                    </form>
                                                @endforeach
                                            </div>
                                        </td>
        ...
        @elseif($sectionKey === 'settings')
            <section class="grid gap-4 xl:grid-cols-3">
                <article class="rounded-[1.8rem] border border-zinc-200 bg-zinc-50 p-5">
                    <div class="text-[11px] uppercase tracking-[0.24em] text-zinc-500">General</div>
                    <form method="POST" action="{{ route('birthdays.settings.save') }}" class="mt-4 grid gap-3">
                        @csrf
                        <input type="hidden" name="scope" value="capture" />
                        <label class="inline-flex items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950">
                            <input type="checkbox" name="year_optional" value="1" @checked(data_get($captureConfig, 'year_optional')) class="rounded border-zinc-300 bg-zinc-50 text-rose-400" />
                            Allow month/day without year
                        </label>
                        <label class="block">
                            <span class="text-xs uppercase tracking-[0.18em] text-zinc-500">Match Priority</span>
                            <input type="text" name="match_priority" value="{{ data_get($captureConfig, 'match_priority') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950" />
                        </label>
                        <label class="block">
                            <span class="text-xs uppercase tracking-[0.18em] text-zinc-500">Required Fields</span>
                            <input type="text" name="required_fields" value="{{ data_get($captureConfig, 'required_fields') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950" />
                        </label>
                        <button type="submit" class="inline-flex rounded-full border border-zinc-300 bg-zinc-50 px-5 py-3 text-sm font-semibold text-zinc-950">Save Capture Rules</button>
                    </form>
                </article>
                <article class="rounded-[1.8rem] border border-zinc-200 bg-zinc-50 p-5">
                    <div class="text-[11px] uppercase tracking-[0.24em] text-zinc-500">Email + SMS</div>
                    <form method="POST" action="{{ route('birthdays.settings.save') }}" class="mt-4 grid gap-3">
                        @csrf
                        <input type="hidden" name="scope" value="campaign" />
                        <label class="inline-flex items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950">
                            <input type="checkbox" name="email_enabled" value="1" @checked(data_get($campaignConfig, 'email_enabled')) class="rounded border-zinc-300 bg-zinc-50 text-rose-400" />
                            Birthday email enabled
                        </label>
                        <label class="inline-flex items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950">
                            <input type="checkbox" name="sms_enabled" value="1" @checked(data_get($campaignConfig, 'sms_enabled')) class="rounded border-zinc-300 bg-zinc-50 text-rose-400" />
                            Birthday SMS enabled
                        </label>
                        <label class="block">
                            <span class="text-xs uppercase tracking-[0.18em] text-zinc-500">Birthday Email Subject</span>
                            <input type="text" name="birthday_email_subject" value="{{ data_get($campaignConfig, 'birthday_email_subject') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950" />
                        </label>
                        <label class="block">
                            <span class="text-xs uppercase tracking-[0.18em] text-zinc-500">Birthday Email Body</span>
                            <textarea name="birthday_email_body" rows="4" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950">{{ data_get($campaignConfig, 'birthday_email_body') }}</textarea>
                        </label>
                        <button type="submit" class="inline-flex rounded-full border border-zinc-300 bg-zinc-50 px-5 py-3 text-sm font-semibold text-zinc-950">Save Messages</button>
                    </form>
                </article>
                <article class="rounded-[1.8rem] border border-zinc-200 bg-zinc-50 p-5">
                    <div class="text-[11px] uppercase tracking-[0.24em] text-zinc-500">Timing</div>
                    <form method="POST" action="{{ route('birthdays.settings.save') }}" class="mt-4 grid gap-3">
                        @csrf
                        <input type="hidden" name="scope" value="campaign" />
                        <label class="block">
                            <span class="text-xs uppercase tracking-[0.18em] text-zinc-500">Birthday Send Offset</span>
                            <input type="number" name="birthday_send_offset" value="{{ data_get($campaignConfig, 'birthday_send_offset') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950" />
                        </label>
                        <label class="block">
                            <span class="text-xs uppercase tracking-[0.18em] text-zinc-500">Follow-up Offset</span>
                            <input type="number" name="followup_send_offset" value="{{ data_get($campaignConfig, 'followup_send_offset') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950" />
                        </label>
                        <label class="block">
                            <span class="text-xs uppercase tracking-[0.18em] text-zinc-500">Follow-up Subject</span>
                            <input type="text" name="followup_email_subject" value="{{ data_get($campaignConfig, 'followup_email_subject') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950" />
                        </label>
                        <label class="block">
                            <span class="text-xs uppercase tracking-[0.18em] text-zinc-500">Follow-up Body</span>
                            <textarea name="followup_email_body" rows="4" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-950">{{ data_get($campaignConfig, 'followup_email_body') }}</textarea>
                        </label>
                        <button type="submit" class="inline-flex rounded-full border border-zinc-300 bg-zinc-50 px-5 py-3 text-sm font-semibold text-zinc-950">Save Timing</button>
                    </form>
                </article>
            </section>
                        ...
                        @foreach($recentRewardSignals as $signal)
                            <div class="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
                                ...
                                @if(data_get($signal->meta, 'reward_code'))
                                    <div class="mt-1 font-mono text-[11px] text-zinc-500">{{ data_get($signal->meta, 'reward_code') }}</div>
                                @endif
                                <div class="mt-1 text-xs text-zinc-500">{{ optional($signal->occurred_at)->format('Y-m-d H:i') ?: '—' }}</div>
                            </div>
                        @endforeach
                    </div>
