<nav aria-label="Automation workspace" class="flex flex-wrap items-center gap-1 rounded-2xl border border-zinc-200 bg-white/80 p-1.5 shadow-sm backdrop-blur">
    @foreach([
        ...
        ['route' => 'workflows.create', 'parameters' => ['picker' => 'templates'], 'label' => 'Templates', 'match' => 'workflows.create'],
    ] as $item)
        <a href="{{ route($item['route'], $item['parameters'] ?? []) }}" wire:navigate class="rounded-xl px-3.5 py-2 text-sm font-semibold transition {{ request()->routeIs($item['match']) ? 'bg-zinc-950 text-white shadow-sm' : 'text-zinc-600 hover:bg-zinc-100 hover:text-zinc-950' }}">{{ $item['label'] }}</a>
    @endforeach
</nav>
