...
<section class="rounded-[2rem] border border-zinc-200 bg-white p-5 sm:p-6 shadow-sm">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <a href="{{ route('marketing.overview') }}" wire:navigate class="inline-flex items-center rounded-full border border-zinc-300 bg-white px-3 py-1.5 text-xs font-semibold text-zinc-700 hover:bg-zinc-50">
            Open Overview
        </a>
    </div>
</section>
...
<section class="rounded-[2rem] border border-zinc-200 bg-white p-4 sm:p-5 shadow-sm">
    ...
    <div class="mt-4 grid gap-3 md:grid-cols-2 2xl:grid-cols-4">
        @foreach($sectionGroups as $group)
            ...
            @endphp
            <article class="rounded-[1.55rem] border p-3 sm:p-4 shadow-sm {{ $group['current'] ? $accent['panel_current'] : $accent['panel'] }}">
