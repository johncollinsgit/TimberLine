        ...
        <div class="mt-4 flex flex-wrap items-center justify-end gap-2">
          @if((int)($draftSummary['line_count'] ?? 0) > 0)
            <button
              ...
              class="rounded-xl border border-rose-300/25 bg-rose-500/10 px-4 py-3 text-sm font-semibold text-rose-50 transition hover:bg-rose-500/16 disabled:cursor-not-allowed disabled:opacity-50"
            >
              Reset Draft
            </button>
          @endif
      ...
      <div class="rounded-2xl border border-emerald-200/10 bg-black/15 p-4">
        ...
        <div class="mt-4 grid grid-cols-12 items-end gap-4">
          <div class="col-span-12 lg:col-span-8">
            ...
            <div class="mt-2 text-[11px] text-emerald-100/50">
              @if(!$selectedScentId)
          ...
          <div class="col-span-12 lg:col-span-4 lg:min-w-[15rem]">
            <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-1">
              <button
                ...
                class="h-12 w-full whitespace-nowrap rounded-2xl border border-emerald-300/25 bg-emerald-500/10 px-4 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"
              >
                Add 0.5 Box
              </button>
              <button
                ...
                @disabled(!$upcomingEventId || !$selectedScentId)
                class="h-12 w-full whitespace-nowrap rounded-2xl border border-emerald-300/35 bg-emerald-500/22 px-4 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"
              >
                Add 1 Box
              </button>
              <button
                ...
                class="h-12 w-full whitespace-nowrap rounded-2xl border border-amber-300/25 bg-amber-500/15 px-4 text-sm font-semibold text-amber-50 shadow-[0_8px_30px_-18px_rgba(251,191,36,0.7)] disabled:cursor-not-allowed disabled:opacity-50 sm:col-span-2 lg:col-span-1"
              >
                Add Top Shelf (16oz)
              </button>
            </div>
      ...
      <div class="rounded-2xl border border-emerald-200/10 bg-black/15 p-4">
        ...
        <div class="mt-4 flex flex-wrap items-center justify-between gap-2">
          <button
            ...
            class="rounded-xl border border-rose-300/25 bg-rose-500/10 px-4 py-2.5 text-xs font-semibold text-rose-50 transition hover:bg-rose-500/16 disabled:cursor-not-allowed disabled:opacity-40"
          >
            Reset Draft
          </button>
  ...
  @if($step === 4)
    <div class="mt-5 rounded-2xl border border-emerald-200/10 bg-black/15 p-4">
      ...
      <div class="mt-2 text-sm text-emerald-50/75">
        ...
        @elseif((int)($draftSummary['line_count'] ?? 0) > 0)
          <span class="text-white">Existing event draft</span>
        @elseif($startFresh)
      ...
      <div class="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <div class="rounded-2xl border border-white/8 bg-black/20 p-4">
          <div class="text-[10px] uppercase tracking-[0.22em] text-emerald-100/45">Standard Lines</div>
          <div class="mt-2 text-2xl font-semibold text-white">{{ (int)($draftSummary['standard_line_count'] ?? 0) }}</div>
        </div>
        <div class="rounded-2xl border border-white/8 bg-black/20 p-4">
          <div class="text-[10px] uppercase tracking-[0.22em] text-emerald-100/45">Top Shelf Lines</div>
          <div class="mt-2 text-2xl font-semibold text-white">{{ (int)($draftSummary['top_shelf_line_count'] ?? 0) }}</div>
        </div>
      </div>

      <div class="mt-4 rounded-2xl border border-emerald-200/10 bg-black/20 p-4">
        <div class="text-[10px] uppercase tracking-[0.22em] text-emerald-100/45">Candle Breakdown</div>
        @if(empty($draftSummary['breakdown']))
          <div class="mt-2 text-sm text-emerald-100/55">No candle rows available yet.</div>
        @else
          <div class="mt-3 space-y-2">
            @foreach(($draftSummary['breakdown'] ?? []) as $row)
              ...
              <div class="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-white/8 bg-white/5 px-3 py-2 text-sm text-emerald-50/90">
                <div class="font-medium text-white">{{ $row['name'] ?? 'Unknown scent' }}</div>
                <div class="flex items-center gap-3 text-xs text-emerald-100/70">
      ...
      <div class="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div class="flex flex-wrap items-center gap-2">
          <button
            ...
            class="rounded-xl border border-white/12 bg-white/5 px-4 py-2.5 text-xs font-semibold text-white/90 hover:bg-white/10"
          >
            Back To Match
          </button>
          <button
