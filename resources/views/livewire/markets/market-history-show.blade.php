  ...
  <section class="rounded-3xl border border-white/10 bg-white/5 p-5 sm:p-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div>
        ...
        <h1 class="mt-2 text-2xl sm:text-3xl font-semibold text-white">{{ $displayTitle }}</h1>
        @if(!blank($rawTitleKey))
          <div class="mt-2 inline-flex items-center rounded-full border border-amber-300/20 bg-amber-400/10 px-3 py-1 text-[11px] text-amber-100/90">
        ...
        @foreach($events as $event)
          <div class="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
            <div class="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
              <div class="min-w-0">
                <div class="text-sm font-semibold text-white">{{ $event->name ?: ($event->display_name ?: 'Untitled Event') }}</div>
