<div class="space-y-6">
  <section class="rounded-3xl border border-emerald-200/10 bg-[#101513]/80 p-6">
    ...
    <div class="mt-2 text-3xl font-['Fraunces'] font-semibold text-white">{{ $list->title }}</div>
    <div class="mt-2 text-sm text-emerald-50/70">Status: {{ $list->status }}</div>
  </section>

  <section class="rounded-3xl border border-emerald-200/10 bg-[#101513]/80 p-6 space-y-4">
    <div class="text-xs text-emerald-100/60">Events in list</div>
    ...
    <div class="flex items-center gap-3">
      <div class="flex items-center gap-2 text-[11px] text-white/60">
        Growth
        <input type="number" step="0.01" wire:model="growthFactor" class="w-20 rounded-xl border border-emerald-200/10 bg-black/20 px-3 py-2 text-xs text-white/90" />
      </div>
      <div class="flex items-center gap-2 text-[11px] text-white/60">
        Safety
        <input type="number" step="0.01" wire:model="safetyStock" class="w-20 rounded-xl border border-emerald-200/10 bg-black/20 px-3 py-2 text-xs text-white/90" />
      </div>
  ...
  <section class="rounded-3xl border border-emerald-200/10 bg-[#101513]/80 p-6">
    <div class="text-xs text-emerald-100/60">Aggregated Lines</div>
    <div class="mt-3 rounded-2xl border border-emerald-200/10 overflow-hidden">
    ...
    <div class="mt-3 text-xs text-emerald-100/60">Total units: {{ $totalQty }}</div>
    <div class="mt-4">
  ...
  <section class="rounded-3xl border border-emerald-200/10 bg-[#101513]/80 p-6">
    <div class="text-xs text-emerald-100/60">Per-Event Lines</div>
    <div class="mt-3 rounded-2xl border border-emerald-200/10 overflow-hidden">
      <div class="grid grid-cols-6 gap-0 bg-black/30 text-[11px] text-white/50 px-3 py-2">
        <div>Event</div>
        <div>Scent</div>
      ...
      <div class="divide-y divide-emerald-200/10">
        @foreach($eventLines as $line)
          <div class="grid grid-cols-6 gap-0 px-3 py-2 text-xs text-white/80">
            ...
            <div class="text-white/40">Event line</div>
          </div>
        @endforeach
      </div>
    </div>
