...
<div class="space-y-6" data-dashboard-root wire:ignore.self>
  <section class="rounded-3xl border border-emerald-200/10 bg-[#101513]/80 p-6 shadow-[0_30px_80px_-50px_rgba(0,0,0,0.9)]">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      <div>
        ...
        <div class="mt-2 text-3xl font-['Fraunces'] font-semibold text-white">{{ $this->greeting }}</div>
      </div>
      <div class="flex flex-col items-start gap-3 md:items-end">
        <div class="max-w-xl text-sm text-emerald-50/70">
          “{{ $quote['quote'] ?? '' }}”
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <select wire:model.live="range" class="rounded-full border border-emerald-300/35 bg-emerald-500/20 px-4 py-2 text-sm font-medium text-white focus:border-emerald-200/70 focus:outline-none">
          ...
          <select wire:model.live="channel" class="rounded-full border border-emerald-300/35 bg-emerald-500/20 px-4 py-2 text-sm font-medium text-white focus:border-emerald-200/70 focus:outline-none">
            <option value="all">All</option>
            <option value="retail">Retail</option>
            <option value="wholesale">Wholesale</option>
          </select>
          <button type="button" wire:click="toggleLibrary"
            class="ml-2 inline-flex items-center rounded-full border px-4 py-2 text-sm font-semibold transition {{ $showLibrary ? 'border-emerald-300/60 bg-emerald-500/35 text-white' : 'border-emerald-200/25 bg-emerald-500/10 text-emerald-100/90 hover:bg-emerald-500/20' }}">
            Widgets Tray
          </button>
        </div>
      </div>
  ...
  <div class="grid grid-cols-1 gap-6 md:grid-cols-12" data-widget-grid>
    @foreach($visible as $w)
      ...
      <section class="{{ $span }} min-h-[230px] cursor-move rounded-2xl border border-emerald-200/10 bg-[#0f1412]/80 p-5 shadow-[0_20px_60px_-40px_rgba(0,0,0,0.9)]"
        data-widget
        data-widget-id="{{ $id }}">
