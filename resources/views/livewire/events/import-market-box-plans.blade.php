<div class="space-y-6">
  <section class="rounded-3xl border border-emerald-200/10 bg-[#101513]/80 p-6">
    ...
    <div class="mt-2 text-3xl font-['Fraunces'] font-semibold text-white">Import Market Box Plans</div>
    <div class="mt-2 text-sm text-emerald-50/70">Upload the normalized master CSV to backfill historical event instances and their box plan lines.</div>
  </section>
  ...
  <section class="rounded-3xl border border-emerald-200/10 bg-[#101513]/80 p-6 space-y-4">
    ...
    @if(!empty($report['import_batch_id']))
      <div class="rounded-2xl border border-emerald-300/15 bg-emerald-500/8 p-4 text-xs text-emerald-50/85">
    ...
    @if(!empty($report['deleted_batch_id']))
      <div class="rounded-2xl border border-amber-300/15 bg-amber-500/8 p-4 text-xs text-amber-50/90">
  ...
  <section class="rounded-3xl border border-emerald-200/10 bg-[#101513]/80 p-6 space-y-4">
    <div class="text-xs text-emerald-100/60">Delete Import Batch</div>
    <div class="flex flex-col gap-3 md:flex-row md:items-center">
      ...
      <button
        ...
        class="rounded-full border border-rose-300/20 bg-rose-500/10 px-4 py-2 text-xs text-rose-100 disabled:cursor-not-allowed disabled:opacity-50"
      >
