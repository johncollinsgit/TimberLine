<div class="space-y-6">
  <section class="rounded-3xl border border-emerald-200/10 bg-[#101513]/80 p-6">
    <div class="text-[11px] uppercase tracking-[0.35em] text-emerald-100/60">Event</div>
    <div class="mt-2 text-3xl font-['Fraunces'] font-semibold text-white">{{ $event->name }}</div>
    <div class="mt-2 text-sm text-emerald-50/70">{{ $event->starts_at }} – {{ $event->ends_at }}</div>
  </section>

  <section class="rounded-3xl border border-emerald-200/10 bg-[#101513]/80 p-6">
    <div class="flex gap-2">
      <button wire:click="$set('tab','overview')" class="px-3 py-2 rounded-full text-xs border {{ $tab==='overview' ? 'border-emerald-300/40 bg-emerald-400/20 text-emerald-50' : 'border-white/10 text-white/70' }}">Overview</button>
      <button wire:click="$set('tab','shipments')" class="px-3 py-2 rounded-full text-xs border {{ $tab==='shipments' ? 'border-emerald-300/40 bg-emerald-400/20 text-emerald-50' : 'border-white/10 text-white/70' }}">Shipment Plan</button>
      <button wire:click="$set('tab','results')" class="px-3 py-2 rounded-full text-xs border {{ $tab==='results' ? 'border-emerald-300/40 bg-emerald-400/20 text-emerald-50' : 'border-white/10 text-white/70' }}">Results</button>
      <button wire:click="$set('tab','recommendations')" class="px-3 py-2 rounded-full text-xs border {{ $tab==='recommendations' ? 'border-emerald-300/40 bg-emerald-400/20 text-emerald-50' : 'border-white/10 text-white/70' }}">Recommendations</button>
    </div>
    ...
    @if($tab==='overview')
      ...
      <div class="mt-3 flex items-center gap-2">
        <button wire:click="saveEvent" class="rounded-full border border-emerald-400/25 bg-emerald-500/15 px-4 py-2 text-xs text-white/90">Save Event</button>
        <button wire:click="createMarketPourList" class="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-xs text-white/80">Generate Market Pour List</button>
      </div>
    @elseif($tab==='shipments')
      <div class="mt-4 grid grid-cols-1 gap-3 md:grid-cols-4">
        <select wire:model="shipmentScentId" class="rounded-xl border border-emerald-200/10 bg-black/20 px-3 py-2 text-sm text-white/90">
          <option value="">Scent</option>
          @foreach($scents as $s)
            <option value="{{ $s->id }}">{{ $s->name }}</option>
          @endforeach
        </select>
        <select wire:model="shipmentSizeId" class="rounded-xl border border-emerald-200/10 bg-black/20 px-3 py-2 text-sm text-white/90">
          <option value="">Size</option>
          @foreach($sizes as $s)
            <option value="{{ $s->id }}">{{ $s->label ?? $s->code }}</option>
          @endforeach
        </select>
        <input type="number" wire:model="shipmentQty" class="rounded-xl border border-emerald-200/10 bg-black/20 px-3 py-2 text-sm text-white/90" placeholder="Qty" />
        <button wire:click="addShipment" class="rounded-xl border border-emerald-400/25 bg-emerald-500/15 px-3 py-2 text-sm text-white/90">Add</button>
      </div>
      <div class="mt-4">
        @foreach($shipments as $row)
          <div class="grid grid-cols-5 gap-2 text-xs text-white/80 items-center">
    ...
    @elseif($tab==='recommendations')
      <div class="mt-4">
        @forelse($recommendations as $r)
          <div class="rounded-xl border border-emerald-200/10 bg-black/20 px-3 py-2 text-xs text-white/80 mb-2">
            <div class="flex items-center justify-between">
              <div>
                <div class="font-semibold">{{ $r['scent_name'] ?? 'Unknown scent' }}</div>
                <div class="text-[11px] text-white/50">{{ $r['size_label'] ?? 'Unknown size' }}</div>
              </div>
              <div class="text-sm text-emerald-50">{{ $r['recommended_qty'] }}</div>
            </div>
        ...
        @empty
          <div class="text-xs text-white/50">No recommendations yet. Add shipment history or seed demo data.</div>
        @endforelse
        <button wire:click="createMarketPourList" class="mt-3 rounded-full border border-emerald-400/25 bg-emerald-500/15 px-4 py-2 text-xs text-white/90">
