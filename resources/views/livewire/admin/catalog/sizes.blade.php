<section class="rounded-3xl border border-zinc-200 bg-zinc-50 p-5">
  <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
    <div>
      <div class="text-lg font-semibold text-zinc-950">Sizes</div>
      <div class="text-sm text-zinc-600">Canonical sizes and pricing.</div>
    </div>
  ...
  @if($showCreate)
    <form wire:submit.prevent="create" class="mt-4 grid gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 md:grid-cols-6">
      ...
      <tbody class="divide-y divide-zinc-200">
        @foreach($sizes as $size)
          <tr class="hover:bg-zinc-50">
            <td class="px-4 py-3 text-zinc-950">{{ $size->label ?: '—' }}</td>
            <td class="px-4 py-3 text-zinc-700">{{ $size->code }}</td>
            <td class="px-4 py-3 text-zinc-700">{{ $size->wholesale_price !== null ? '$'.number_format($size->wholesale_price, 2) : '—' }}</td>
            <td class="px-4 py-3 text-zinc-700">{{ $size->retail_price !== null ? '$'.number_format($size->retail_price, 2) : '—' }}</td>
            <td class="px-4 py-3">
              <span class="inline-flex rounded-full px-2 py-1 text-xs {{ $size->is_active ? 'bg-emerald-100 text-emerald-900' : 'bg-zinc-100 text-zinc-500' }}">
                {{ $size->is_active ? 'Active' : 'Inactive' }}
              </span>
            </td>
            <td class="px-4 py-3 text-right space-x-2">
              <button type="button" wire:click="openEdit({{ $size->id }})" class="rounded-full border border-emerald-400/30 bg-emerald-100 px-3 py-1 text-[11px] text-emerald-900">Edit</button>
  ...
  @if($showEdit)
    <div class="fixed inset-0 z-[9999] flex items-center justify-center fb-overlay-soft p-4" style="position: fixed; inset: 0; z-index: 99999;" data-admin-modal>
      <div class="w-full max-w-2xl rounded-2xl border border-zinc-200 bg-white p-6">
        <div class="text-lg font-semibold text-zinc-950">Edit Size</div>
  ...
  @if($showDelete)
    <div class="fixed inset-0 z-[9999] flex items-center justify-center fb-overlay-soft p-4" style="position: fixed; inset: 0; z-index: 99999;" data-admin-modal>
      <div class="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6">
        <div class="text-lg font-semibold text-zinc-950">Delete Size</div>
