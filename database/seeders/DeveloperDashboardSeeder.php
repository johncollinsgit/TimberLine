...
class DeveloperDashboardSeeder extends Seeder
{
    ...
    private function checklistItems(): array
    {
        return [
            ...
            ['slug' => 'tenant-isolation', 'label' => 'Enforced tenant data isolation', 'category' => 'Security', 'status' => 'partial', 'detail' => 'The five Tier-1 cross-tenant IDOR sites (pouring, shipping, mapping, candle-cash, identity-review) are now closed — each scoped to the acting user\'s memberships with cross-tenant + flagship-preservation tests. A gated global-scope backstop (BelongsToTenant + TenantScope + TenantContext) is built and proven, and orders/marketing_profiles are backfilled. Remaining: backfill every BelongsToTenant table, then arm FEATURE_ENFORCED_TENANT_SCOPE.'],

            ['slug' => 'module-entitlement-gate', 'label' => 'Enforced module entitlements', 'category' => 'Multi-tenant', 'status' => 'done', 'detail' => 'Shipped: a module:{key} route middleware reads TenantModuleAccessResolver\'s decision and 403s at the request layer, applied to field-service as the reference. New module routes opt in with one line — entitlements are no longer nav-only.'],
            ...
            ['slug' => 'per-tenant-integrations', 'label' => 'Per-tenant integration connections', 'category' => 'Multi-tenant', 'status' => 'partial', 'detail' => 'The normalized foundation shipped: an integration_connections table (tenant-owned, encrypted tokens, unique per provider/account) behind a ProviderConnector interface + ConnectionManager registry + refresh command. Remaining: migrate the four bespoke flows (Shopify, Google Business, Square, Asana) onto it via Socialite so each tenant connects their own accounts. Email/SendGrid already does this.'],
    ...
    private function changes(): array
    {
        return [
            [
                'slug' => 'tenant-isolation-hardening',
                'title' => 'Closed the 5 cross-tenant IDOR sites + built the isolation backstop',
                'category' => 'security',
                'status' => 'shipped',
                'impact' => 'high',
                'reference' => 'CrossTenantIdorGuardTest · EnforcedTenantScopeTest',
                'changed_at' => '2026-07-07 16:30:00',
                'summary' => "The keystone security work. Closed all five Tier-1 cross-tenant IDOR sites — pouring stack, shipping editor, mapping exceptions, candle-cash customer adjust (a financial IDOR that could credit/debit another tenant's reward balance), and identity-review merge. Each is now scoped to the acting user's memberships, with tests proving cross-tenant access is blocked AND the caller's own records still work unchanged. Also built a gated global-scope backstop (BelongsToTenant + TenantScope + a scoped TenantContext + forAllTenants() escape hatch), proven on IntegrationConnection, and backfilled tenant_id onto orders + marketing_profiles. The backstop stays OFF (FEATURE_ENFORCED_TENANT_SCOPE) until every tenant-owned table is backfilled. Full suite green (1894 tests).",
            ],
            [
                'slug' => 'integration-connections-foundation',
                'title' => 'Per-tenant integration_connections foundation (OAuth standard)',
                'category' => 'architecture',
                'status' => 'shipped',
                'impact' => 'high',
                'reference' => 'integration_connections · ConnectionManager',
                'changed_at' => '2026-07-07 15:30:00',
                'summary' => "Answered 'is there an OAuth library for e-commerce?' — no universal one exists; the professional standard is Laravel Socialite + a normalized per-tenant connections table + a refresh job. Built exactly that foundation: an integration_connections table (tenant-owned, encrypted access/refresh tokens, unique per provider/account), a ProviderConnector interface, a ConnectionManager registry, and an integrations:refresh-connections command. Additive only — nothing reads from it yet, so the four existing bespoke OAuth flows are untouched. This is the store every provider (Shopify/Square/QuickBooks/Google) migrates onto so each tenant connects their own accounts.",
            ],
            [
                'slug' => 'module-entitlement-gate-live',
                'title' => 'Module entitlement enforcement went live',
                'category' => 'architecture',
                'status' => 'shipped',
                'impact' => 'high',
                'reference' => 'PR #18 · EnsureModuleAccess',
                'changed_at' => '2026-07-07 05:09:00',
                'summary' => 'Shipped the module:{key} route gate (PR #18, deployed through the CI gate). module_catalog + TenantModuleAccessResolver already produced a correct per-module decision, but nothing enforced it — every route was role-gated only and canAccess() just hid nav, so any role-holder could reach any module by URL. Now a single middleware enforces the decision at the request layer (applied to field-service as the behavior-preserving reference; new module routes opt in with one line). Also shipped config:doctor and the .env.example repair in the same increment.',
            ],
            [
    ...
    private function ideas(): array
    {
        return [
            ...
            [
                ...
                'status' => 'done',
                'pitch' => "THE move that turns discipline into architecture. Shipped: the five Tier-1 IDOR sites are closed (membership-scoped, with cross-tenant + flagship-preservation tests), and a gated global-scope backstop (BelongsToTenant + TenantScope + a scoped TenantContext + a forAllTenants() escape hatch) is built and proven on IntegrationConnection. It PROTECTS the untested candle modules without redesigning them. Arming it platform-wide is the follow-up (see 'Arm the enforced tenant scope').",
            ],
            [
                ...
                'status' => 'done',
                'pitch' => 'Shipped: a module:{key} route middleware reads TenantModuleAccessResolver\'s decision and 403s at the request layer (applied to field-service as the reference; new module routes opt in with one line). Entitlements are no longer nav-only — the flagship candle modules are now inaccessible to tenants without the candle entitlement. The decision engine already existed; this wired it in.',
            ],
            [
                ...
                'status' => 'done',
                'pitch' => "Foundation shipped: one integration_connections table (tenant_id, provider, encrypted access/refresh tokens, unique per provider/account) behind a ProviderConnector interface + ConnectionManager registry + a refresh command — modeled on the already-correct per-tenant email pattern. Answers 'is there an OAuth library for e-commerce?': no universal one exists; the standard is Socialite + a normalized connections table, which this establishes. Follow-up: migrate the four bespoke flows (Shopify/GBP/Square/Asana) onto it so every tenant connects their own accounts.",
            ],
            [
                'slug' => 'standard-module-contract',
            ...
            [
                ...
                'title' => 'Add tenant_id to the operational tables that lack it',
                'impact' => 'medium',
                'effort' => 'medium',
                'category' => 'security',
                'source' => 'Module audit · ~11 tables have no tenant_id',
                'status' => 'proposed',
                'pitch' => 'orders and marketing_profiles are now backfilled to the flagship. Still architecturally single-tenant: the markets/events/pouring/retail tables (retail_plans, markets, market_plans, events, event_instances, pour_batches, pour_requests, pouring_measurements) have no tenant_id column at all. Add tenant_id + BelongsToTenant with a backfill to the flagship, so those modules can be scoped too. This is the remaining prerequisite before arming the enforced global scope platform-wide.',
            ],
            [
                'slug' => 'arm-enforced-tenant-scope',
                'title' => 'Arm the enforced tenant scope platform-wide',
                'impact' => 'high',
                'effort' => 'medium',
                'category' => 'security',
                'source' => 'Isolation backstop · built but gated off',
                'status' => 'proposed',
                'pitch' => 'The enforced global tenant scope is built, tested, and gated behind FEATURE_ENFORCED_TENANT_SCOPE (off) — it filters every BelongsToTenant model to the acting tenant, fail-open when no context is set. Before flipping it on: give every tenant-owned table a fully backfilled tenant_id (orders + marketing_profiles done; the operational tables still need columns), adopt BelongsToTenant on those models, then arm the flag behind a canary and watch for any flagship query returning empty. Arming it is what makes isolation default-on rather than per-query discipline.',
            ],
            [
            ...
            [
                ...
                'status' => 'done',
                'pitch' => 'Shipped: .env.example now carries the required SHOPIFY_RETAIL_* and SQUARE_* blocks, and a config:doctor command asserts required keys per environment (fails loudly on gaps). It immediately caught a real production issue — MAIL_MAILER=log, so the default mailer sends no real email (blocks password reset / email verification). That env fix is a pending owner action.',
            ],
            [
                'slug' => 'gdpr-redaction',
