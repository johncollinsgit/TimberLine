...
return new class extends Migration
{
    ...
    {
        if (! Schema::hasTable('tenant_messaging_accounts')) {
        ...
        if (! Schema::hasTable('tenant_messaging_sender_profiles')) {
            Schema::create('tenant_messaging_sender_profiles', function (Blueprint $table): void {
                ...
                $table->foreign('tenant_messaging_account_id', 'tm_sender_account_fk')
                    ->references('id')->on('tenant_messaging_accounts')->nullOnDelete();
        ...
        if (! Schema::hasTable('tenant_messaging_credit_accounts')) {
        ...
        if (! Schema::hasTable('tenant_messaging_usage_periods')) {
        ...
        if (! Schema::hasTable('tenant_messaging_ledger_entries')) {
            Schema::create('tenant_messaging_ledger_entries', function (Blueprint $table): void {
                ...
                $table->foreign('tenant_messaging_credit_account_id', 'tm_ledger_credit_fk')
                    ->references('id')->on('tenant_messaging_credit_accounts')->nullOnDelete();
                $table->foreignId('tenant_messaging_usage_period_id')->nullable();
                $table->foreign('tenant_messaging_usage_period_id', 'tm_ledger_period_fk')
                    ->references('id')->on('tenant_messaging_usage_periods')->nullOnDelete();
        ...
        $this->addAttributionColumn('source_module_key', fn (Blueprint $table) => $table->string('source_module_key', 80)->nullable()->index()->after('channel'));
        $this->addAttributionColumn('source_campaign_id', fn (Blueprint $table) => $table->unsignedBigInteger('source_campaign_id')->nullable()->after('source_module_key'));
        $this->addAttributionColumn('source_campaign_label', fn (Blueprint $table) => $table->string('source_campaign_label', 180)->nullable()->after('source_campaign_id'));
        $this->addAttributionColumn('attribution_type', fn (Blueprint $table) => $table->string('attribution_type', 20)->default('direct')->index()->after('attribution_model'));
        $this->addAttributionColumn('confidence_percent', fn (Blueprint $table) => $table->unsignedTinyInteger('confidence_percent')->default(100)->after('attribution_type'));
        $this->addAttributionColumn('currency_code', fn (Blueprint $table) => $table->string('currency_code', 8)->default('USD')->after('revenue_cents'));
        $this->addAttributionColumn('gross_revenue_cents', fn (Blueprint $table) => $table->integer('gross_revenue_cents')->default(0)->after('currency_code'));
        $this->addAttributionColumn('refund_cents', fn (Blueprint $table) => $table->integer('refund_cents')->default(0)->after('gross_revenue_cents'));
        $this->addAttributionColumn('net_revenue_cents', fn (Blueprint $table) => $table->integer('net_revenue_cents')->default(0)->after('refund_cents'));
        $this->addAttributionColumn('provider_cost_micros', fn (Blueprint $table) => $table->unsignedBigInteger('provider_cost_micros')->default(0)->after('net_revenue_cents'));
        $this->addAttributionColumn('buyer_spend_micros', fn (Blueprint $table) => $table->unsignedBigInteger('buyer_spend_micros')->default(0)->after('provider_cost_micros'));
    }

    protected function addAttributionColumn(string $column, Closure $definition): void
    {
        if (! Schema::hasColumn('marketing_message_order_attributions', $column)) {
            Schema::table('marketing_message_order_attributions', $definition);
        }
    }

    public function down(): void
    {
        if (Schema::hasColumn('marketing_message_order_attributions', 'source_module_key')) {
