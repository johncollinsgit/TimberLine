...
- Drawings are PDF files selected from iOS Files or Documents, limited to 50 MiB by default on the resumable path, stored as authenticated tenant/job assets, audited, and team-visible by default. The phone uses a stable UUID initialization key and sends retryable 512 KiB chunks during a two-hour upload session; the server accepts completion only after exact-size, per-chunk checksum, whole-file checksum, portable structural PDF checks, and bounded Ghostscript parsing where configured. Legacy multipart/base64 requests are one file and 25 MiB maximum. Active-session and staging-byte quotas bound unfinished uploads, cleanup retains records when deletion cannot be verified, and a completion lease prevents cancellation/cleanup races. Authorized Collins employees request a five-minute tenant/asset-bound preview URL so iOS can page local PDFs with byte ranges instead of buffering the full file; URL issuance is audited once and the signed route is not a public asset URL.
...
- A read-only QuickBooks invoice sync promotes only clear whole-generator installations into customer equipment. It retains generator inlets, cords, interlocks, transfer-switch-only work, and ambiguous records in the historical audit for owner review. An unambiguous annual generator-service invoice creates completed service history, sets last service to the invoice date, and sets the next annual service date one year later. See `docs/operations/collins-generator-invoice-history-2026-09-03.md`.
