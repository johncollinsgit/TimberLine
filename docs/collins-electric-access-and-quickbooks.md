...
- Do not enable payments, write-back, webhooks, CDC, Estimator, or payroll features from discovery alone. The hourly scheduler may be enabled for Collins only after the dry-run, backup, two stable live imports, mapping review, and owner/team permission smoke tests pass.
...
- The shared `documents` Branch stores authenticated private copies. Team uploads default to team-visible; QuickBooks attachments default to owner-only. One asset can link to several jobs, text/CSV contents and tags are searchable, and upload/link/download/delete activity is audited. iCloud remains a source picker, not canonical storage.
- The shared `estimator` Branch is default-disabled and owner/admin-only. It proposes candidates only from repeated detailed invoice descriptions, excludes broad service items, snapshots approved prices into Everbranch drafts, and never writes estimates back to QuickBooks.
