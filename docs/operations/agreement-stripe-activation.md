...
- The landlord Transactions ledger reads the configured Stripe account's Payment Intents API as its primary source, including incomplete attempts, with a 30-second cache and an operator refresh action. It counts only succeeded intents with a positive `amount_received` as received. If the API is unavailable, it falls back to processed same-mode Stripe invoice webhooks while excluding validation-only agreement orders and unverified/non-Stripe rows.
- The landlord `Invoices` sidebar destination manages direct-invoice drafts and sent invoices. For a mapped open direct invoice, invoice detail and the transaction ledger can send a payment reminder to an encrypted billing phone. Each send requires an operator to confirm express billing-text consent, rejects a known SMS opt-out, re-retrieves the Stripe invoice, and proceeds only with Stripe's current open status, positive amount remaining, and HTTPS `hosted_invoice_url`. The message identifies Everbranch and includes STOP instructions; the messaging usage ledger and invoice metadata enforce idempotency, and operator audit records retain only a phone hash/last four.
- Transaction itemization must reconcile exactly to the Stripe-confirmed subtotal. Future agreement phases are excluded from the current receipt, and direct-invoice extended amounts must not be multiplied by quantity a second time.
...
`AgreementPaymentConfirmationMail` provides an operator-sent, branded HTML/plain-text confirmation for an already paid receipt belonging to the same tenant and agreement. It requires a paid timestamp and provider event reference, uses the provider-confirmed total, and itemizes the original order only when those lines reconcile to the receipt subtotal. It does not add an automatic webhook email or replay prior sends. The paid proposal presents confirmation and receipt access instead of another acceptance/payment prompt.
