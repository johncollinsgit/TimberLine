...
- Production agreement-checkout allowlisting uses the confirmation-gated `maintenance-allow-agreement-checkout.yml` workflow. It preserves the complete Forge environment, rejects wildcard allowlists, adds one validated tenant slug idempotently, refreshes the Laravel config cache, and verifies the unsigned agreement readiness without sending or charging it.
