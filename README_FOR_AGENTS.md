...
- Everbranch app 2.1.0 consumes bootstrap contract v3 plus Field Service contract v5. Say Branch in tenant-facing discovery copy, keep canonical `module_key` identifiers internally, return `branches`, and preserve `modules` plus `/modules/...` only as compatibility aliases.
...
- Field Operations v3 clock, channel, invitation, material, fleet, candidate, and asset-upload mutations must remain tenant-scoped and idempotent. Employees can read only participating jobs and attached contacts; owner financial context must be omitted server-side for every other role. New asset writes use the configured private workspace disk and retain dual local reads until migration is complete.
