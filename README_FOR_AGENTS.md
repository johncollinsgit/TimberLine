...
- Production now has a Forge zero-downtime site with shared `storage`, retained
  ...
  mandatory. Its post-gate production action is a Forge deploy-hook POST using
...
- The Forge production site tracks `main` and is configured for zero-downtime
  ...
  the protected Forge hook after that gate succeeds; routine SSH deployment is
  retired and available only as explicitly approved emergency recovery.
...
- Uploaded tenant brand marks resolve through the public application asset route so atomic releases do not depend on a web-server `/storage` symlink. Branding is public identity data only; private workspace files continue to use authenticated asset delivery.
...
- `fleet_tracking` is a separate default-disabled, internal-only tenant add-on. Its global flag, module entitlement/dependencies, counsel-review record, policy version/hash, employee acknowledgement, active timer, and signed Bouncie webhook verification are independent fail-closed gates. Keep company-van Bouncie points separate from employee phone points; enforce a 30-day maximum raw retention, owner/admin-only viewing, and no personal/off-duty tracking, speed scoring, geofence alerting, or automated employment decisions. Follow `docs/operations/field-operations-location-tracking-runbook.md` for rollout and rollback.
- The fictional Green Shield Pest Control sales fixture is created only through `everbranch:prepare-pest-control-demo`; it is idempotent, isolates its public demo user to that tenant, and never represents a real customer, vehicle, policy, location, or legal review. Its public password is intentionally non-secret and documented only for this demo. The authenticated fleet surface remains fail-closed until `FLEET_TRACKING_ENABLED` and all ordinary rollout gates are complete.
