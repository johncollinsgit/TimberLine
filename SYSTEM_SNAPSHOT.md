...
- Landlord direct Stripe invoices now exist for approved Everbranch/Evergrove one-time, milestone, and supplemental charges. They are tenant-scoped, draft-first, separately feature-flagged from proposal Checkout, exclude Shopify and third-party provider expenses, send through Stripe hosted invoices, mirror Stripe-confirmed receipt/tax/status evidence, and never mutate module access or plan entitlements. The landlord sidebar links directly to invoice creation/management. Eligible open invoices can receive an idempotent, audited SMS reminder only after an operator confirms billing-text consent; the server re-retrieves Stripe's current open status, positive amount remaining, and HTTPS hosted invoice URL, blocks known opt-outs, and keeps the billing phone encrypted at rest.
