...
- Collins Electric guided launch pass (2026-07-11): Collins Electric is intentionally a hand-held launch-partner workspace, not a 3-day trial or billing activation. `php artisan everbranch:prepare-collins-electric --seed-demo-job` creates/updates tenant `collins-electric`, applies the `electrician` blueprint on direct/base, records setup interests including QuickBooks/uploads/billing as interests only, finalizes safe modules only (`customers`, `field_service`, `messaging`, `reporting`), and attaches `johncollinsemail@gmail.com` as active verified admin for Everbranch mobile testing. Electrician field service now supports lock box code, job updates/notes, note-linked photos, calendar view, job detail, tenant-scoped search, mobile job detail/add-note/photo linkage, and disabled/not-verified reminder settings. QuickBooks is CSV/XLSX concierge import through `field-service:import-quickbooks`; Apple Photos remain manual job photo import; SMS sends must stay off until provider readiness, consent, and delivery logs are smoke-tested.
