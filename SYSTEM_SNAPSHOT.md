...
- Modern Forestry app completion pass (2026-06-19, updated 2026-06-23): tenant `1` now has a Shopify embedded `Edit App` page with draft/publish App Content snapshots, top tabs for Customer Dashboard and Mobile Home editing, plus a signed app-proxy customer dashboard at `/shopify/marketing/v1/account` for linked Shopify customer identities. The dashboard shows published copy only, rewards, status, recent orders, Shopify cart reorder handoff, support, privacy, terms, and data request actions. Email/phone lookup remains limited to rewards lookup and does not expose orders/activity.
