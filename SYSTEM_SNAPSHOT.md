...
- Collins Electric guided launch and QuickBooks discovery pass (2026-07-13): Collins Electric remains a hand-held launch-partner workspace, not a trial or billing activation. QuickBooks is a shared opt-in beta Branch, not Collins-specific code: onboarding records interest only; an owner/admin entitlement plus tenant-scoped OAuth is required before audit/import commands run. The production OAuth connection and full read-only audit are verified. The audit confirms substantial customer, estimate, invoice, purchase, repeated-price, receivable, wage, and contract-labor signals; exact customer and accounting records remain encrypted/tenant-scoped and are not committed to source. Imports preserve invoices/estimates/lines/attachments as separate financial documents and use `QuickBooksJobEvidenceClassifier`; a line description alone enters owner review instead of creating a field job. Private invoice notes and financial search/detail context are owner/admin-only; team members retain customers, jobs, calendar, assignments, operational notes/photos, and search without financial reports or totals. P&L wages and contract labor are owner-only aggregates, not employee payroll records. Estimator, payments, write-back, webhooks, CDC, scheduled sync, payroll, and SMS remain off.
