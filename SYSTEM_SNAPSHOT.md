...
- Everbranch Work mobile API foundation (2026-07-03): a generic internal mobile API now lives at `/api/mobile/work/v1/*`, separate from the Modern Forestry branded customer app routes. It uses magic-link challenges plus hashed bearer sessions, maps users through `tenant_user`, auto-selects single-tenant users, returns a tenant picker for multi-tenant users, and generates mobile tabs from module access. V1 mobile data reuses `MarketingProfile`, field-service jobs/tasks, and tenant users; Modern Forestry `/api/mobile/v1/modern-forestry/*` behavior remains unchanged.
- Everbranch Work collaboration foundation (2026-07-03): the Work mobile API now includes employee-focused notification preferences, in-app notifications, email/push delivery audit rows, user-based Work push devices, direct team notify, job/task comments, explicit mentions, watchers, and activity timelines. These primitives are tenant/user scoped over existing field-service jobs/tasks and remain separate from Modern Forestry customer push infrastructure.
- Everbranch Work app launch slice (2026-07-03): `everbranch-work-app/` is now the React Native/Expo iOS+Android app shell for internal tenant teams. The launch navigation is intentionally Home, Jobs, Team only. The API now includes `/home`, job status updates, job/task filters, admin-only team invites, role-aware mobile permissions, and Expo push delivery attempts.
- First-login Trades workspace pass (2026-07-04): verified users with no tenant now get a short self-serve confirm step that creates a Trades workspace immediately, seeds the tenant from the Trades/Electrical/Plumbing blueprint family, and lands the user in the new tenant without landlord approval. The legacy electrician template remains as a backward-compatible alias.
- Everbranch Work electrician vertical pass (2026-07-03): the shared Work app now leans into electrician jobs/projects with site addresses, phone numbers, notes, lock-box codes, photos, task-stage timelines, and comment threads. Local demo data in the Everbranch Work tenant seeds Carlos, Andrew, and Neal so the team and collaboration surfaces have real content during phone testing.
...
- Android and iOS mobile app readiness are product requirements. Current mobile APIs are split between the Modern Forestry branded customer app surface and the generic internal Everbranch Work surface for tenant teams.
