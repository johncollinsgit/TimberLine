...
- Field Operations workforce and tracking foundation (2026-08-13): `time_tracking` now supports tenant-scoped assigned shifts, optional enforced clock windows, employee timer-correction requests, manager resolution, audit evidence, and the existing approved-hours CSV handoff only (never payroll/tax calculation or payment). `fleet_tracking` is a separate default-disabled internal add-on with independent Bouncie company-vehicle and active-timer phone sources, 30-day maximum raw retention, owner/admin-only access, policy/counsel/acknowledgement gates, idempotent signed webhook ingestion, and a global kill switch. See `docs/operations/field-operations-location-tracking-runbook.md`; no personal/off-duty tracking, speed scoring, geofence alerts, or automated employment decisions are in V1.
- Green Shield Pest Control demo (2026-08-13): `everbranch:prepare-pest-control-demo` creates an idempotent, tenant-isolated fictional field-operations workspace with an explicitly public demo account, scheduled pest-control work, a Bouncie-mapped fictional van, and separate fictional location points. `/platform/demos/green-shield-pest-control` hosts the matching 30-second self-hosted Remotion tour; its demo-login handoff prefills the public email and returns a successful password sign-in to that workspace. The fixture is not production evidence and the actual Fleet Tracking page remains behind the global rollout switch and all normal policy gates.
