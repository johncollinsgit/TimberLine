...
- Collins Electric guided launch and QuickBooks discovery pass (2026-07-13): Collins Electric remains a hand-held launch-partner workspace, not a trial or billing activation. QuickBooks is a shared opt-in beta Branch, not Collins-specific code: onboarding records interest only; an owner/admin entitlement plus tenant-scoped OAuth is required before audit/import commands run. The read-only full audit stores encrypted source snapshots and aggregate summaries, preserves invoices/estimates/lines/attachments in separate tenant models, links only operationally evidenced jobs, keeps manual jobs intact, and imports QuickBooks services into a reusable price book. Private invoice notes and financial search context are owner/admin-only; team members retain customers, jobs, calendar, assignments, operational notes/photos, and search without financial reports or totals. P&L wages and contract labor are owner-only aggregates; the user-reported roughly $100,000 Collins contract-labor YTD remains unverified until the production API audit confirms amount and period. Estimator, payments, write-back, webhooks, CDC, scheduled sync, payroll, and SMS remain off.
...
- Live sync reads Customers, Estimates, Invoices, Items, and attachment metadata into tenant-scoped customers, financial documents/lines, operationally evidenced jobs, source-attributed notes, attachment metadata, and price-book items. It does not treat every accounting document as a job or map service items into physical materials.
