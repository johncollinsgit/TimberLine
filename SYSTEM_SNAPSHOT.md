...
- Everbranch Product Options remains a Shopify-only, tenant-scoped module.
  Assigned option sets render automatically through the already enabled theme
  app embed as well as the optional product block, so a missing block placement
  cannot leave a configured bundle without selectors.
- Normal Add to cart and accelerated product checkout controls are blocked in
  the browser until all required `Scent 1...N` values are present and any
  distinct-value rule passes.
- The `everbranch-bundle-scent-validation` Shopify Function is the
  ...
  It reads Everbranch rule JSON from each assigned product's
  `everbranch.bundle_scent_rule` metafield, with migration fallbacks for the
  initially assigned Modern Forestry bundle handles.
- Product Option create/update/delete actions synchronize those product
  metafields through the tenant-owned installed Shopify store. The
  `shopify:sync-product-option-validation --tenant-id={id}` command previews
  the initial/backfill sync and requires `--apply` for writes.
...
- Landlord direct Stripe invoices now exist for approved Everbranch/Evergrove one-time, milestone, and supplemental charges. They are tenant-scoped, draft-first, separately feature-flagged from proposal Checkout, exclude Shopify and third-party provider expenses, and support explicit email, text-only, or combined delivery choices. Text-only delivery finalizes a non-auto-advancing Stripe invoice to obtain its hosted payment link before using the consent-confirmed SMS path. Provider-confirmed receipt/tax/status evidence remains authoritative, and invoice activity never mutates module access or plan entitlements. Eligible open invoices can receive an idempotent, audited SMS reminder only after an operator confirms billing-text consent; the server re-retrieves Stripe's current open status, positive amount remaining, and HTTPS hosted invoice URL, blocks known opt-outs, and keeps the billing phone encrypted at rest.
- Tenant Team Access is membership-scoped, never a global user directory. It lists only the active tenant's members and tenant-addressed `platform_access` requests, updates pivot roles inline, and removes only the current membership. Wholesale applications are available only inside the configured wholesale tenant's dedicated inbox; generic platform approvals cannot invoke Shopify wholesale synchronization.
...
- Base Workspace + Field Service module foundation (2026-07-02): Everbranch now has a direct `base` plan for non-Shopify tenants and a real `field_service` module with tenant-scoped jobs, tasks, materials, photo/file links, and vehicles. Direct blueprint provisioning uses the base plan, and field-service tenants see a clean Work surface instead of Modern Forestry candle-production navigation.
- Collins Electric guided launch and QuickBooks reporting pass (2026-07-13): Collins Electric remains a hand-held launch-partner workspace, not a trial or billing activation. QuickBooks is a shared opt-in beta Branch, not Collins-specific code: onboarding records interest only; an owner/admin entitlement plus tenant-scoped OAuth is required before audit/import commands run. Imports preserve invoices/estimates/payments/purchases/bills/lines/attachments as separate financial documents and use `QuickBooksJobEvidenceClassifier`; a line description alone enters owner review instead of creating a field job. Owner reporting adds unpaid/overdue invoices, mapped supplies, wages with and without Nathan's compensation, contract and combined labor percentages based on P&L Total Income, work billed, completed jobs, customer concentration, quote aging, sync health, and prior-year comparisons. Team members receive operational Reporting and upcoming jobs but no financial cards. The shared Documents Branch stores private tenant assets, multi-job links, searchable tags/text, owner/team visibility, and audited access; the mobile app uses the iOS photo picker to copy selected iCloud photos into Everbranch. The shared Estimator Branch is default-disabled, owner-only, uses reviewed repeated-description candidates, and creates Everbranch drafts with no QuickBooks writes. Payments, write-back, webhooks, CDC, payroll, and SMS remain off. Hourly read-only sync is per-connection opt-in and must pass the Collins dry-run/backup/idempotency/permission rollout gate before activation.
...
- PR 2 brand/navigation ownership evidence lives in `docs/operations/everbranch-route-page-ownership-inventory.md`; shared display labels live in `config/everbranch.php`.
- PR 3 added the first tenant setup status skeleton on `/start` plus landlord onboarding diagnostics visibility. It captures Shopify/Square/CSV/manual import intent, safe module interests, and Android/iOS mobile interest without activating checkout, connector automation, or generic mobile APIs.
...
- PR 11 added conservative Shopify privacy webhook readiness for `customers/data_request`, `customers/redact`, and `shop/redact`. Valid HMAC requests create `shopify_privacy_webhook_events` records with payload hashes and minimal summaries, defaulting to manual review required. No destructive customer/shop/tenant deletion or anonymization is automated yet; Partner Dashboard/CLI deployment evidence remains required.
- PR 12 added `docs/operations/shopify-partner-dashboard-evidence-runbook.md` plus `ShopifyPartnerEvidenceRunbookTest`. The runbook documents exact Shopify CLI deploy, Partner Dashboard checklist, dev-store install/reinstall, privacy webhook delivery, app proxy, scope review, privacy manual review, and evidence storage steps. Live external Shopify evidence remains pending until a human operator executes the runbook.
- PR 13 added plan selection without billing. Tenant `/start` captures plan interest, billing lane interest, implementation help interest, and commercial notes as intent only; landlord intake shows and triages that commercial intent. No checkout, billing, subscriptions, quotes, invoices, module installs, or entitlement changes are activated.
- PR 14 added landlord `/landlord/commercial-intent` as an operator-only commercial intent summary and billing lane decision gate. It shows plan/lane grouping, implementation help, custom request counts, Shopify evidence blockers, disabled billing blockers, and review-only commercial updates. No checkout, billing, subscriptions, quotes, invoices, module installs, or entitlement changes are activated.
- PR 15 added Shopify external evidence packet `docs/operations/evidence/shopify/2026-05-21/README.md` and scope/branding decision record `docs/operations/shopify-scope-branding-decision-record.md`. Shopify CLI is present locally, but no app deploy/release, Partner Dashboard verification, dev-store install/reinstall, app proxy, or live privacy webhook delivery evidence was completed. Scope and app name/handle decisions remain pending.
- PR 16 added landlord `/landlord/readiness` as an operator-only self-service readiness dashboard. It summarizes onboarding, intake, modules, custom requests, commercial intent, billing, Shopify app, privacy webhooks, external evidence, mobile, and launch readiness; it does not approve launch or activate billing/modules/mobile.
- PR 17 confirmed the current Shopify Partner/dev dashboard target app is `Modern Forestry Backstage`, handle `modernforestrybackstage`, and dev store `modernforestry.myshopify.com`. Use that app/store for current evidence capture. Everbranch public Shopify app branding remains a later decision.
- PR 18 captured partial external Shopify evidence under `docs/operations/evidence/shopify/2026-05-21/`: read-only `shopify app info` confirms the target app/dev store, app-proxy health works through the Modern Forestry primary storefront, and scope-review evidence is documented. Partner Dashboard screenshots, deploy/release output, dev-store install/reinstall, and live privacy webhook delivery rows remain pending.
- PR 19 prepared the final Shopify operator screenshot pack: `screenshot-manifest.md` and `operator-checklist.md` under `docs/operations/evidence/shopify/2026-05-21/`. It does not complete external evidence, deploy/release Shopify config, trigger webhooks, change scopes, or activate billing.
- PR 20 adds the Everbranch logo asset set and wires Laravel-rendered platform branding to config-backed Everbranch assets. Shopify TOML/Partner app identity remains `Modern Forestry Backstage`; Modern Forestry tenant-specific copy remains preserved.
- Electrician onboarding modal pass: incomplete first-sign-in tenants now land on `/start`, auto-open a branded modal onboarding shell, and complete a shorter electrician-first wizard that only exposes safe visible module choices. The same `/onboarding` contract/store remain canonical, completion now unlocks the workspace when the final blueprint exists, and optional client brand preferences promote display name/logo URL into `tenant_discovery_profiles` on finalize.
- Customer electrician tutorial visibility pass (2026-07-01): the customer/public electrician tutorial exposure is now hidden by default behind `FEATURE_CUSTOMER_ELECTRICIAN_TUTORIAL=false`. Public promo/access-request electrician choices, tenant `/start` electrician onboarding teaser/modal, tenant `/onboarding`, and the tenant `Setup plan` quick action are all suppressed while the flag is off, but the underlying `electrician` template and landlord/internal onboarding tooling remain intact for later restoration.
- Access-surface separation pass: post-login routing now recognizes true `platform_admin` users as landlord operators, completed tenant users land in the tenant app, incomplete tenant users land on `/start`, and demo/sandbox account modes show explicit tenant-app banners. The authenticated blueprint wizard also has a landlord-only `/landlord/onboarding/wizard` entry and no longer presents ambiguous "Set up your tenant" copy.
- Test access/seed pass: `php artisan everbranch:seed-access-surfaces` now creates or updates safe local/staging access records for a platform admin, Modern Forestry tenant admin, Everbranch demo tenant/user, and sandbox tenant/user. Landlord tenant detail pages show a read-only Test Access panel with lane warnings and available test-user emails. Direct impersonation remains deferred; no login bypass or password reset action is active.
- Tenant blueprint foundation pass: landlord `/landlord/tenants/create` can create a tenant with a config-backed business template, operating mode, data source preference, presentation labels, starter module recommendations, setup notes, and next action. Blueprint state is stored in `TenantAccessProfile.metadata.tenant_blueprint` and reflected through existing `tenant_setup_statuses`; `/start` now shows the blueprint so non-Shopify tenants do not read as Shopify-only. Blueprints are planning/setup metadata only and do not install modules, change entitlements, start billing, trigger Shopify OAuth, or automate Square/CSV imports.
- Work management blueprint intent pass: tenant blueprints now capture requested/planned project/work tracking, task management, user assignments, team/client communication, photo/file uploads, mobile field capture, and project/task/assignee/communication/upload labels. These fields are config/service-backed intent only; no Projects, Tasks, Assignments, Comments, Messaging, Photo/File Upload, Mobile Capture, Notification, billing, entitlement, storage, or mobile API workflows are active.
- Blueprint edit/review pass: landlord operators can edit existing tenant blueprints at `/landlord/tenants/{tenant}/blueprint/edit`, update safe profile/work-management fields, and mark blueprints `unreviewed`, `needs_follow_up`, `reviewed`, or `archived`. Review metadata stays in `TenantAccessProfile.metadata.tenant_blueprint`; `blueprint_internal_notes` and `blueprint_next_action` are landlord-only, while tenant `/start` only reflects tenant-facing blueprint/setup guidance.
- Blueprint-driven module catalog alignment pass: `TenantBlueprintModuleRecommendationService` now derives display-only module recommendations from tenant blueprint templates, operating mode, starter modules, setup module interests, and work-management intent. Tenant Module Store, landlord tenant detail, and tenant `/start` show recommended/requested/planned/future/not-active-yet states without creating modules, installing modules, changing entitlements, activating billing, running imports, enabling uploads/messaging, or creating mobile APIs.
- Workflow automation activation pass (2026-06-01): Everbranch now includes a first-party scheduler-driven workflow engine with an Asana -> Google Calendar driver to replace the existing Zapier flow for Modern Forestry. Access is tenant-scoped through the `workflow_automations` module and the default entitlement path is explicitly tied to Modern Forestry tenant `1`, not plan-wide rollout.
- Everbranch brand system and language sweep pass: `config/everbranch.php` now carries Evergrove/Everbranch identity, brand assets, brand tokens, and display-language labels. Public, tenant, landlord, auth/shared, and safe Shopify embedded copy has started moving from generic Backstage and technical terms toward Everbranch, workspace, setup, feature, access, status, and plan-interest language. Shopify Partner/TOML identity remains `Modern Forestry Backstage` / `modernforestrybackstage`; no billing, checkout, deploy, OAuth, scope, module, entitlement, import, mobile, or privacy-deletion behavior was activated.
- Backstage visual-brand retirement pass (2026-07-11): obsolete `public/brand/forestry-backstage-*` assets were removed, Everbranch fallback copy now uses Everbranch instead of Forestry Backstage, and visible account/review/Shopify embedded copy was updated away from Backstage language. This does **not** rename or remove Shopify. The active Modern Forestry Shopify app TOML/Partner identity remains `Modern Forestry Backstage` / `modernforestrybackstage` until the Shopify scope/branding decision record is approved and matching Partner Dashboard evidence is captured.
- Modern Forestry app completion pass (2026-06-19, updated 2026-06-23): tenant `1` now has a Shopify embedded `Edit App` page with draft/publish App Content snapshots, top tabs for Customer Dashboard and Mobile Home editing, a live phone-shaped Mobile Home preview, plus a signed app-proxy customer dashboard at `/shopify/marketing/v1/account` for linked Shopify customer identities. The dashboard shows published copy only, rewards, status, recent orders, Shopify cart reorder handoff, support, privacy, terms, and data request actions. Email/phone lookup remains limited to rewards lookup and does not expose orders/activity.
...
- Everbranch mobile tenant headers resolve audited admin-managed discovery branding, with a tenant-name fallback and Modern Forestry tenant-1 wordmark fallback. Authorized operators switch between isolated tenant and landlord contexts without reauthentication; Landlord Home reports catalog-derived MRR, tenant mix/growth/activity, users, Branch readiness, and triage queues, while tenant Home receives only its own workspace pulse.
- Mobile support tickets are a base tenant capability with threaded tenant replies and a separate audited landlord queue. Landlord navigation is Home, Tenants, Tickets, Reports, and Account, so tenant Work/Branches controls never appear in landlord context.
- Trades mobile Home now reports in-progress jobs, their gross/contract value, distinct working crews, and potential/estimate/quoted jobs from tenant-scoped field data. Branches mobile discovery counts all non-active paths and returns a specific icon plus expandable product language for active and available Branches.
...
- Landlord/admin Phase 1 host foundation is implemented:
  ...
  - canonical landlord host target in production is `app.theeverbranch.com` (configurable)
  - canonical tenant host target in production is `<slug>.theeverbranch.com`
  - legacy domains are edge-redirect sources only; runtime host acceptance is canonical-only
  - unknown/unexpected hosts are rejected safely (`404`) with no first-tenant fallback
...
- Production DNS/TLS verification completed on 2026-03-27:
  - canonical Everbranch cert should cover `theeverbranch.com`, `app.theeverbranch.com`, and `*.theeverbranch.com`
  - legacy domains are redirected at Cloudflare with path/query preservation
...
## Shared QuickBooks Branch + Collins First Use (2026-07-13)

- QuickBooks Online is the first real shared accounting Branch through `QuickBooksConnector`; it is enabled per tenant through the canonical entitlement resolver. Collins is the first use, not a fork.
- Full discovery starts with `php artisan field-service:audit-quickbooks --tenant=collins-electric --full --dry-run`. A non-dry audit stores encrypted source snapshots and aggregate findings without logging raw records.
- Live sync reads Customers, Estimates, Invoices, Items, and attachment metadata into tenant-scoped customers, financial documents/lines, operationally evidenced jobs, source-attributed notes, attachment metadata, and price-book items. Job evidence is limited to QuickBooks job/subcustomers, project references, distinct service addresses, explicit private/customer memos, or service-dated lines. Description-only documents remain owner-searchable review records; service items do not become physical materials.
