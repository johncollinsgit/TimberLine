...
test('customer onboarding quick action stays hidden while the electrician tutorial feature is off', function (): void {
    $service = new class(Mockery::mock(AuthenticatedTenantContextResolver::class), Mockery::mock(TenantExperienceProfileService::class), Mockery::mock(TenantModuleAccessResolver::class), Mockery::mock(TenantHostBuilder::class)) extends UnifiedAppNavigationService
    {
...
test('Website Products stays directly below Website after personalized ordering', function (): void {
    $service = new class(Mockery::mock(AuthenticatedTenantContextResolver::class), Mockery::mock(TenantExperienceProfileService::class), Mockery::mock(TenantModuleAccessResolver::class), Mockery::mock(TenantHostBuilder::class)) extends UnifiedAppNavigationService
    {
        public function exposedPinProducts(array $items): array
        {
            return $this->pinWebsiteProductsBelowWebsite($items);
        }
    };

    $items = $service->exposedPinProducts([
    ...
    expect(array_column($items, 'key'))->toBe(['home', 'branches', 'managed-website', 'website-products', 'administration']);
});
