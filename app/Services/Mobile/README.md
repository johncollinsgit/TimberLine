...
Field Service contract v4 is the Collins-first Work 2.0 API. It preserves the existing field-service tables and adds computed readiness, profile/viewer capabilities, My Day, transitions, richer tasks, notifications, and typed destinations. Only `field_service` entitlement metadata `experience_version=2` activates the new trades client. Keep all authorization in Laravel and keep Everbranch APNs (`com.everbranch.app`) separate from Modern Forestry push infrastructure.
