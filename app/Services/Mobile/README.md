...
Collins Electric mobile testing is tenant-membership driven. `johncollinsemail@gmail.com` signs in through `/mobile/authorize` PKCE and receives Collins because of an admin membership; `collinselectric91@gmail.com` is the Collins owner. Field-service Work payloads include customer phone, schedule, service address, lock box, lead/participants, tasks, activity/comments/mentions, authenticated photos/documents, and owner-only financial detail. The client must not infer a tenant or activate SMS, billing, or imports from setup interests.
...
For field-service tenants, bottom Work opens the canonical Field Service contract v3. Calendar is the default view and includes selected-day work plus an unscheduled tray. List filters are My Jobs, Active, Quotes, and History. `work_core` is omitted when Field Service is enabled; legacy `/work` and generic module paths remain compatibility aliases. Member reads are server-scoped to lead assignment, participation/following, task assignment, or mention.
...
Field Service photos use multipart mobile uploads and authenticated asset downloads. The iOS system picker may select from iCloud or Shared Albums, but Everbranch stores a private copy and never crawls the album. Collins' QuickBooks import supplied no attachments, so Documents should explain the empty state and direct the team to upload from a job.
...
# Field Operations v6 contract

Field Service contract v6 and My Day v5 are reusable tenant contracts. They preserve the legacy primary task assignee for compatibility while returning canonical `assignees[]`, an assigned-to-me cursor feed, shared completion, waiting-state handoffs, idempotent task history, and owner-only period metrics. Bootstrap contract v3 accepts the client’s `X-Everbranch-App-Version` and hides modules whose minimum version or role contract is not satisfied. Keep authorization and financial filtering in Laravel, and keep Everbranch APNs (`com.everbranch.app`) separate from Modern Forestry push infrastructure.
