...
Collins Electric mobile testing is tenant-membership driven. `johncollinsemail@gmail.com` should sign in through the normal `/mobile/authorize` PKCE flow, then `/api/mobile/v1/workspaces` should include `collins-electric` only because the user has an admin membership. Field-service Work payloads include customer phone, scheduled date, lock box presence/detail, notes/activity updates, note-linked photos, and tenant-scoped add-note/photo actions. The client must not infer a tenant or activate SMS, billing, or imports from setup interests.
