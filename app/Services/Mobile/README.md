...
Field Service Files separates Photos from Drawings & PDFs. Photos use the system photo picker; PDFs use the system Files picker, accept only PDF, enforce 25 MB on client and server, store an authenticated tenant/job-scoped copy, record asset audit history, default to team visibility, and preview inline. Team visibility allows authorized coworkers to read the asset but never creates a public URL.
