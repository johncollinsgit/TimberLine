...
Collins Electric mobile testing is tenant-membership driven. `johncollinsemail@gmail.com` signs in through `/mobile/authorize` PKCE and receives Collins because of an admin membership; `collinselectric91@gmail.com` is the Collins owner. Field-service Work payloads include customer phone, structured service address, external PM contact, schedule, lock box, lead/participants, tasks, activity/comments/mentions, and authenticated team files. Financial detail is loaded only after the owner/admin gate succeeds. The client must not infer a tenant or activate SMS, billing, or imports from setup interests.
