...
Customer reads remain available through the entitled Customers Branch. Customer create and update require `mobile:write` plus a server-resolved owner/admin role; the client permission flag is presentation only. Delete is intentionally narrower and succeeds only for a `mobile_manual` profile with no provider link, external profile, group, consent event, delivery, Candle Cash, birthday, field job, or messaging conversation. All three mutations are tenant-scoped and recorded as landlord operator actions. Imported, connected, and historical customer records are preserved.
...
Collins Upstate Electric mobile visibility is tenant-membership driven. `johncollinsemail@gmail.com` signs in through `/mobile/authorize` PKCE and receives Collins from `/api/mobile/v1/workspaces` because of an explicit admin membership; `collinselectric91@gmail.com` is the Collins owner. The app must not hardcode the Collins tenant or infer access from either email address. Field-service Work payloads include customer phone, structured service address, external PM contact, schedule, lock box, lead/participants, tasks, activity/comments/mentions, and authenticated team files. Financial detail is loaded only after the owner/admin gate succeeds. The client must not infer a tenant or activate SMS, billing, or imports from setup interests.

QuickBooks is a shared web-owner beta Branch but intentionally has no mobile Branch manifest yet. Do not expose QuickBooks financial documents, totals, receivables, P&L wages, contract labor, price-book costs, or private invoice notes through tenant mobile bootstrap/Work/Search. Team mobile users may receive operational memos and work-line descriptions only. A future native QuickBooks owner view requires its own finite renderer contract, role checks, screenshots, and mobile tests.

Trade-profile Home metrics resolve from tenant-scoped operational lifecycle, not the literal legacy `in_progress` value. Current `active` and `needs_details` work counts; `quote`, `complete`, and records older than one year remain separate. The mobile Branches catalog supplies a purpose-specific icon for every visible entry, counts every non-active growth path, and exposes product/setup language for owned as well as available Branches.

For field-service tenants, bottom Work opens the canonical Field Service contract v7. The Current/Past job inbox is role-aware; Job Drafts are owner/admin-only. `work_core` is omitted when Field Service is enabled, while legacy `/work` and old work-candidate paths remain compatibility aliases for one release. Job reads default to assignment/participation/task/mention scope; entitlement metadata may opt a tenant into `all_operational` reads without broadening mutations.

Job comments create tenant activity and notification-delivery records. SMS remains suppressed unless a tenant staff preference has a verified phone, operational consent, and a verified sender. A queued notification record is not proof that APNs or SMS was delivered.

Job photo uploads accept an optional stable Idempotency-Key. The server scopes it to the tenant, actor, and job and replays one result across multipart retries and the JSON fallback, preventing a lost success response from creating a duplicate asset; clients that omit the header retain the legacy behavior.

Field Service Files separates Photos from Drawings & PDFs. Photos use the system photo picker; PDFs use the system Files picker, accept only PDF, enforce a configurable 50 MiB safety limit on the resumable client and server, store an authenticated tenant/job-scoped copy, record asset audit history, default to team visibility, and preview inline. Initialize sends a stable per-file UUID `Idempotency-Key`, so a lost response can safely return the original session and token without consuming another quota slot. The local-storage API advertises `mode: chunked` and a 512 KiB chunk size, checks the session token, current job-mutation permission, offsets, chunk and file SHA-256 values, exact size, PDF header/object/xref/startxref/EOF structure, and (when available) a bounded safe Ghostscript parse, then completes idempotently through a protected `completing` lease. Legacy multipart/base64 endpoints accept one PDF per request and remain capped at 25 MiB. A client that abandons or exhausts retries deletes `/field-service/uploads/{uploadId}` with the session token so its active quota is released immediately; cancel verifies deletion, never recursively deletes outside the exact staging shape, and remains idempotent if a job assignment changes. Sessions expire after two hours, and scheduled cleanup retains its database row whenever storage deletion cannot be verified. An authorized PDF viewer obtains a five-minute signed `/api/mobile/v1/asset-previews/{tenant}/{asset}` URL; the PDF iframe can then make byte-range requests without buffering an authenticated Blob. URL issuance is audited once, range reads are not, and the tenant/asset-bound signature does not make the underlying asset public.
...
# Field Operations v7 contract

Field Service contract v7 and My Day v5 are reusable tenant contracts. V7 adds owner/admin Job Draft CRUD/archive/restore/publish/link, structured addresses, external PM contacts, office-labelled handoff recipients, idempotent Send to Office, and authenticated team PDF files. Draft and non-financial payloads omit QuickBooks document identifiers, amounts, balances, receivables, and private notes. Bootstrap contract v3 accepts the client’s `X-Everbranch-App-Version`, returns canonical `TenantBrandProfile` presentation, and hides modules whose minimum version or role contract is not satisfied. Keep authorization and financial filtering in Laravel, and keep Everbranch APNs (`com.everbranch.app`) separate from Modern Forestry push infrastructure.
