...
class OperatorDashboardService
{
    ...
    {
        $weekCost = OperatorRecurringCost::query()->where('active', true)->get()->sum(fn ($c) => match ($c->cadence) {
            'weekly' => $c->amount_cents, 'annual' => (int) round($c->amount_cents / 52), default => (int) round($c->amount_cents / 4.345)
        });
