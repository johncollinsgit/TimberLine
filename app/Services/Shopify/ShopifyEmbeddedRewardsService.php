...
class ShopifyEmbeddedRewardsService
{
    ...
    {
        $pointsValue = max(0, (int) ($data['points_value'] ?? 0));
        $rewardAmount = round($this->candleCashService->amountFromPoints($pointsValue), 2);
    ...
    public function updateRedeemRule(CandleCashReward $reward, array $data): array
    {
        ...
        if ($this->candleCashService->isStorefrontReward($reward)) {
            ...
            if ($pointsCost !== $expectedCost) {
                throw ValidationException::withMessages([
                    'points_cost' => 'Storefront Candle Cash cost is derived from the discount value and current points-per-dollar setting.',
                ]);
            }
