...
class ShopifyWholesaleApplicationCustomerService
{
    protected const CUSTOMER_LOOKUP_QUERY = <<<'GRAPHQL'
query FindWholesaleApplicationCustomerByEmail($query: String!) {
...
mutation UpsertWholesaleApplicationCustomer($identifier: CustomerSetIdentifiers, $input: CustomerSetInput!) {
...
GRAPHQL;
    ...
    /**
     * Create or update the Shopify customer record for a wholesale application.
     *
     * This keeps the applicant in native Shopify without applying the wholesale tag.
     *
    ...
    public function syncByEmail(string $email, array $attributes = []): array
    {
        ...
        if ($normalizedEmail === null) {
            throw new RuntimeException('Wholesale application customer sync requires a valid email address.');
        }
        ...
        if ($customerId === null) {
            throw new RuntimeException('Shopify returned a wholesale application customer without a usable customer ID.');
        }
