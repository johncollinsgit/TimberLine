...
class TwilioProviderClient
{
    ...
    public function sendSms(string $toPhone, string $message, array $options = []): array
    {
        ...
        $platformManaged = (bool) ($options['platform_managed'] ?? false)
            && (bool) config('features.tenant_messaging_platform');
        if ((! $enabled || ! $twilioEnabled) && ! $platformManaged) {
            return [
                'success' => false,
                'provider' => 'twilio',
                'provider_message_id' => null,
    ...
    {
        if (! (bool) config('marketing.twilio.verify_signature') && ! app()->environment('production')) {
            return true;
        }
    ...
    {
        if (is_array($options['sender_override'] ?? null)) {
            return ['sender' => $options['sender_override'], 'error' => null];
        }
