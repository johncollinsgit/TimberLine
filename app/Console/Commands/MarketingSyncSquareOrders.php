...
class MarketingSyncSquareOrders extends Command
{
    ...
    public function handle(SquareMarketingSyncService $syncService): int
    {
        ...
        if ($tenantId === null) {
            $this->error('Missing required --tenant-id. Square order sync is tenant-scoped in MT-2D.');
            return self::FAILURE;
        }
        ...
        if ($cursor === null && $resumeRunId !== null) {
            try {
                $cursor = $this->checkpointCursorFromRun($resumeRunId, 'square_orders_sync', $tenantId);
