...
class FleetTrackingController extends Controller
{
    ...
    public function updateSettings(Request $request): RedirectResponse
    {
        $tenant = $this->tenant($request);
        $this->authorizeViewer($request, $tenant);
        $data = $request->validate(['phone_tracking_enabled' => ['nullable', 'boolean'], 'bouncie_tracking_enabled' => ['nullable', 'boolean'], 'policy_version' => ['required', 'string', 'max:80'], 'policy_text' => ['required', 'string', 'max:12000'], 'approval_reference' => ['required', 'string', 'max:500'], 'retention_days' => ['required', 'integer', 'min:1', 'max:30'], 'approval_confirmed' => ['accepted']]);
        $settings = $this->access->settings($tenant);
        $before = $settings->only(['phone_tracking_enabled', 'bouncie_tracking_enabled', 'policy_version', 'policy_sha256', 'approval_basis', 'approval_reference', 'approved_at', 'retention_days']);
        $settings->forceFill(['phone_tracking_enabled' => (bool) ($data['phone_tracking_enabled'] ?? false), 'bouncie_tracking_enabled' => (bool) ($data['bouncie_tracking_enabled'] ?? false), 'policy_version' => $data['policy_version'], 'policy_sha256' => hash('sha256', $data['policy_text']), 'approval_basis' => 'owner', 'approval_reference' => $data['approval_reference'], 'approved_at' => now(), 'approved_by_user_id' => (int) $request->user()->id, 'retention_days' => (int) $data['retention_days']])->save();
        $this->audit->record((int) $tenant->id, (int) $request->user()->id, 'fleet_tracking.settings.updated', targetType: 'tenant_fleet_tracking_setting', targetId: $settings->id, beforeState: $before, afterState: $settings->only(['phone_tracking_enabled', 'bouncie_tracking_enabled', 'policy_version', 'policy_sha256', 'approval_basis', 'approval_reference', 'approved_at', 'retention_days']));
