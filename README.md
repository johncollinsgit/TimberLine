...
- The verified Shopify store also selects the embedded surface before any `/shopify/app` controller or API runs. Confirmed wholesale stores may use only `/shopify/app/wholesale*`; retail HTML routes redirect to Wholesale Overview and retail APIs or mutations return `403`. Retail and `mixed` stores retain the retail surface and cannot open Wholesale Operations.
- Embedded page contexts are retained per verified store so retail and wholesale Shopify Admin tabs cannot overwrite each other. A `store_key` query value may select only a context already verified in the current server session and never establishes tenant, role, module, or store ownership.
...
- `modernforestrywholesale.com`, recognized historical wholesale applications, review workflow, customers, classifications, Places results, and prospect data remain Modern Forestry tenant data. Wholesale applications carry explicit `wholesale_application` provenance; general platform access requests for the same tenant are excluded from the wholesale inbox and decisions. Other tenants see only their own records.
...
- The dedicated wholesale home remains `/shopify/app/wholesale`. Its `<s-app-nav>` emits one `rel="home"` Overview link followed by exactly seven visible sidebar links: Suggestions, Customers, Orders, Follow-Ups, Prospects, Discover, and Applications. Wholesale Customers and Orders use qualified wholesale activity only, including for identities that also buy retail. Subscriptions/Candle Club, Product Options, retail Customers, AI Assistant, Marketing Results, Messages, Rewards, Edit App, Settings, and Module Store remain retail-only.
- Rollout order is strict: deploy Laravel and migrations, restart queue workers, and smoke-test both Shopify installations. Create/release a wholesale Shopify app version with `shopify app deploy --config wholesale --allow-updates` only when TOML or extension configuration changes; do not release a Shopify version against an older backend.
...
- The mobile source of truth is the private `johncollinsgit/everbranch-mobile` repository. Native signing material, provider configuration, and local environment files must never be committed.
...
- Everbranch 1.3.0 uses bottom navigation `Home`, `Work`, `Search`, `Account`, `Branches`. Field-service Work opens Calendar by default, keeps unscheduled imported work visible, and scopes member lists to jobs they lead, participate in, follow, receive as a task, or are mentioned on.
- Tenant bootstrap includes audited workspace branding and a tenant-only workspace pulse. Tenant 1 falls back to the published Modern Forestry wordmark; other tenants show the discovery-profile logo or bold company name. Only tenant admins may change the mobile header.
- Trade Home resolves from the tenant blueprint and tenant-owned field jobs: jobs in progress, their gross revenue, distinct working crews, and potential/estimate/quoted pipeline work. The client cannot select its own vertical or totals.
- Authorized landlord operators switch from a tenant workspace without reauthentication because both contexts use the same revocable device token. Landlord authorization remains separate from tenant membership and exposes its own Home, Tenants, Tickets, Reports, and Account navigation; tenant Work and Branches must never appear there.
- Landlord Home and Reports expose catalog-derived recurring revenue, tenant/user/activity totals, tenant mix, cumulative tenant growth, recent activity, and per-tenant users and Branch readiness. These are landlord-authorized payloads, never additions to tenant bootstrap.
- Tenant support tickets are a base capability rather than a paid Branch. Tenant members may create and reply only inside their workspace; landlord operators may assign, reply, wait, and resolve through the audited landlord queue.
- Every visible Branch has a canonical purpose icon. Active/purchased Branches remain explorable with outcome, fit, setup, and first-workflow language; generic Share controls and inert summaries remain prohibited.
...
- Candle Cash rule: if the customer already has a live issued storefront reward code, the mobile rewards flow should return that reusable code to the iOS app instead of stopping at a blocked warning. The iOS bag must only show a lowered total when Shopify's own cart estimate reflects the discount; do not fake a local subtraction.
...
- Home boot rule: do not put Candle Cash cleanup, checkout recovery, or other bag-repair network work on the root iOS startup path. Keep those repairs tied to explicit Bag / checkout actions so Home stays snappy even when reward state is messy.
...
- Message Analytics now supports `scope=all|direct|campaign` (default `all`) so campaign sends and direct sends can be viewed together or isolated.
- New deterministic repair command `marketing:backfill-message-context` backfills missing tenant/store context on campaign message deliveries + engagement events and can trigger post-repair attribution sync.
